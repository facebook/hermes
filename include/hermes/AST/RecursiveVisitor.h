...
/// Whenever the ExtraT... is provided, the parent must also be used to avoid
/// confusion.
template <typename N, typename V, typename... E>
void visitCaller(V &v, Node **node, Node *parent, E... extra) {
  if constexpr (has_callable_visit<V, N *, Node **, Node *, E...>::value)
    v.visit(static_cast<N *>(*node), node, parent, extra...);
  else if constexpr (has_callable_visit<V, N *, Node **>::value)
    v.visit(static_cast<N *>(*node), node);
  else if constexpr (has_callable_visit<V, N *, Node *, E...>::value)
    v.visit(static_cast<N *>(*node), parent, extra...);
  else
...
/// \section Extra Info
///
/// The dispatcher takes optional type parameters ExtraT,
/// which can be passed as the last arguments to the dispatcher.
/// This allows the visitor to pass information to the recursive calls.
/// The dispatcher will call the appropriate overload of the visitor's
/// visit() method.
///
/// NOTE: visit() methods that take ExtraT must take a parent
/// argument as well, to avoid confusion around whether the parent pointer is
/// extra info or not.
///
/// visitChildren can also be called with ExtraT, which will be propagated to
...
struct RecursiveVisitorDispatch {
  ...
  static void visit(Visitor &v, Node *&nodeRef, Node *parent, ExtraT... extra) {
    auto *node = nodeRef;
    if (!node)
      return;
...
#define VISIT(NAME)                                                 \
  case NodeKind::NAME: {                                            \
    detail::visitCaller<NAME##Node>(v, &nodeRef, parent, extra...); \
    break;                                                          \
  }
...
#undef VISIT
  ...
  static void visit(Visitor &, NodeLabel, Node *, ExtraT...) {}
  static void visit(Visitor &, NodeBoolean, Node *, ExtraT...) {}
  static void visit(Visitor &, NodeNumber &, Node *, ExtraT...) {}
  ...
  static void visit(Visitor &v, NodeList &list, Node *parent, ExtraT... extra) {
...
#define VISIT(NAME)    \
  case NodeKind::NAME: \
    return visitChildrenImpl(v, cast<NAME##Node>(node), extra...);
...
#define ESTREE_NODE_0_ARGS(NAME, BASE)                       \
  HERMES_ALWAYS_INLINE static inline void visitChildrenImpl( \
      Visitor &v, NAME##Node *, ExtraT... extra) {}
...
template <class Visitor, typename ExtraT>
void visitESTreeNodeList(
    Visitor &v,
    NodeList &list,
    Node *parent,
    ExtraT extra) {
  RecursiveVisitorDispatch<Visitor, ExtraT>::visit(v, list, parent, extra);
}

template <class Visitor, typename ExtraT>
void visitESTreeNode(Visitor &v, Node *&node, Node *parent, ExtraT extra) {
  RecursiveVisitorDispatch<Visitor, ExtraT>::visit(v, node, parent, extra);
}
...
/// Recursively visit the children of the node.
template <class Visitor, class Node, typename ExtraT>
void visitESTreeChildren(Visitor &v, Node *node, ExtraT extra) {
  RecursiveVisitorDispatch<Visitor, ExtraT>::visitChildren(v, node, extra);
}
