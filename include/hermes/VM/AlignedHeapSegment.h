...
class FixedSizeHeapSegment : public AlignedHeapSegment {
 public:
  ...
  /// Default to the base type move constructor and assignment operator.
  ...
  /// This is just a variant to provide the same interface as other HeapSegment
  /// type and used in template functions. The extra parameter is always ignored
  /// since FixedSizeHeapSegment has fixed size.
  static llvh::ErrorOr<FixedSizeHeapSegment>
  create(StorageProvider *provider, const char *name, size_t) {
