...
class JSMapImpl final : public JSObject {
 ...
 public:
  ...
  static ExecutionStatus initializeStorage(
    ...
    self->storage_.set(storageHandle.get(), &runtime->getHeap());
  ...
  HashMapEntry *iteratorNext(HashMapEntry *entry) {
    return storage_->iteratorNext(entry);
  }
  ...
  /// \returns the size.
  static uint32_t getSize(JSMapImpl *self) {
    self->assertInitialized();
    return self->storage_->size();
  }

  /// Clear all elements from the storage.
  static void clear(Handle<JSMapImpl> self, Runtime *runtime) {
    self->assertInitialized();
    self->storage_->clear(runtime);
  }
  ...
  static ExecutionStatus forEach(
    ...
    for (entry = self->storage_->iteratorNext(); entry;
         entry = self->storage_->iteratorNext(entry.get())) {
 ...
 public:
  ...
  static CallResult<HermesValue> nextElement(
    ...
    if (!self->iterationFinished_) {
      ...
      self->itr_.set(
          self->data_->iteratorNext(self->itr_), &runtime->getHeap());
      if (self->itr_) {
