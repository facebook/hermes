...
class OrderedHashMap final : public GCCell {
 ...
 public:
  ...
  /// deleted.
  HashMapEntry *iteratorNext(HashMapEntry *entry = nullptr) const;
 ...
 private:
  ...
  void removeLinkedListNode(HashMapEntry *entry, GC *gc);

  /// Lookup an entry with key as \p key in a given \p bucket.
  HashMapEntry *lookupInBucket(uint32_t bucket, HermesValue key);
