...
class JSTypedArrayBase : public JSObject {
 public:
  ...
  Handle<JSArrayBuffer> getBuffer(Runtime *runtime) const {
    return runtime->makeHandle(getBuffer());
  }
  ...
  bool attached() const {
    return buffer_ && buffer_->attached();
  }
  ...
  JSArrayBuffer::size_type getByteOffset() const {
    ...
    return src_ - buffer_->getDataBlock();
  }
  ...
  T &at(size_type i) const {
    assert(attached() && "at() requires a JSArrayBuffer");
