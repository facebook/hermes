...
class Runtime : public HandleRootOwner, private GCBase::GCCallbacks {
 public:
  ...
  // Gets the cppPropertyCache_ entry with the specified index.
  inline PropertyCacheEntry *getCppPropertyCacheEntry(uint32_t idx) {
    return &cppPropertyCache_[idx];
  }

  // Sets the cppPropertyCache_ capacity to at least size.
  inline void setCppPropertyCacheSize(uint32_t size) {
    cppPropertyCache_.resize(size);
  }
#endif
...
#ifdef HERMESVM_CPP_RUNTIME
  /// The read/write property cache for functions generated by the CPP backend
  std::vector<PropertyCacheEntry> cppPropertyCache_{};
#endif
