...
class ArrayImpl : public JSObject {
 ...
 public:
  ...
  static void unsafeSetExistingElementAt(
    ...
    self->indexedStorage_->at(index - self->beginIndex_)
 ...
 protected:
  ...
  const HermesValue unsafeAt(size_type index) const {
    return indexedStorage_->at(index - beginIndex_);
  }
