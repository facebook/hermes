...
llvh::Optional<std::nullptr_t> Object::getNull(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsNull();
  return llvh::None;
}
llvh::Optional<bool> Object::getBoolean(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsBoolean();
  return llvh::None;
}
llvh::Optional<double> Object::getNumber(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsNumber();
  return llvh::None;
}
llvh::Optional<int64_t> Object::getInteger(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsInteger();
  return llvh::None;
}
llvh::Optional<llvh::StringRef> Object::getString(StringRef K) const {
  if (auto *V = get(K))
...
bool Parser::parseError(const char *Msg) {
  ...
  Err.emplace(
      llvh::make_unique<ParseError>(Msg, Line, P - StartOfLine, P - Start));
  return false;
}
} // namespace
...
bool isUTF8(llvh::StringRef S, size_t *ErrOffset) {
...
static void quote(llvh::raw_ostream &OS, llvh::StringRef S) {
  OS << '\"';
...
void llvh::json::OStream::value(const Value &V) {
  switch (V.kind()) {
  case Value::Null:
    valueBegin();
    OS << "null";
...
void llvh::json::OStream::valueBegin() {
...
void llvh::json::OStream::newline() {
  if (IndentSize) {
    OS.write('\n');
    OS.indent(Indent);
  }
...
void llvh::json::OStream::arrayBegin() {
  valueBegin();
  Stack.emplace_back();
...
void llvh::json::OStream::arrayEnd() {
...
void llvh::json::OStream::objectBegin() {
  valueBegin();
  Stack.emplace_back();
...
void llvh::json::OStream::objectEnd() {
...
void llvh::json::OStream::attributeBegin(llvh::StringRef Key) {
  assert(Stack.back().Ctx == Object);
  if (Stack.back().HasValue)
    OS << ',';
  newline();
...
void llvh::json::OStream::attributeEnd() {
...
void llvh::format_provider<llvh::json::Value>::format(
    const llvh::json::Value &E, raw_ostream &OS, StringRef Options) {
