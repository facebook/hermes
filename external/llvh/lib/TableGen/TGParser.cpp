...
Init *TGParser::ParseSimpleValue(Record *CurRec, RecTy *ItemType,
  ...
  case tgtok::l_paren: {         // Value ::= '(' IDValue DagArgList ')'
    ...
    SmallVector<std::pair<llvh::Init*, StringInit*>, 8> DagArgs;
    if (Lex.getCode() != tgtok::r_paren) {
...
void TGParser::ParseDagArgList(
    SmallVectorImpl<std::pair<llvh::Init*, StringInit*>> &Result,
    Record *CurRec) {
...
bool TGParser::ParseForeach(MultiClass *CurMultiClass) {
  ...
  Loops.push_back(llvh::make_unique<ForeachLoop>(Loc, IterName, ListValue));
...
bool TGParser::ParseClass() {
  ...
  if (CurRec) {
    ...
    auto NewRec =
        llvh::make_unique<Record>(Lex.getCurStrVal(), Lex.getLoc(), Records,
                                  /*Class=*/true);
    CurRec = NewRec.get();
...
bool TGParser::ParseMultiClass() {
  ...
  auto Result =
    MultiClasses.insert(std::make_pair(Name,
                    llvh::make_unique<MultiClass>(Name, Lex.getLoc(),Records)));
