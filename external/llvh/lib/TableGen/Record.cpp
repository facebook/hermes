...
RecordRecTy *RecordRecTy::get(ArrayRef<Record *> UnsortedClasses) {
  ...
  SmallVector<Record *, 4> Classes(UnsortedClasses.begin(),
                                   UnsortedClasses.end());
  llvh::sort(Classes, [](Record *LHS, Record *RHS) {
...
bool RecordRecTy::isSubClassOf(Record *Class) const {
  return llvh::any_of(getClasses(), [Class](Record *MySuperClass) {
...
bool RecordRecTy::typeIsConvertibleTo(const RecTy *RHS) const {
  ...
  return llvh::all_of(RTy->getClasses(), [this](Record *TargetClass) {
...
RecTy *llvh::resolveTypes(RecTy *T1, RecTy *T2) {
  if (T1 == T2)
    return T1;
...
raw_ostream &llvh::operator<<(raw_ostream &OS, const Record &R) {
...
raw_ostream &llvh::operator<<(raw_ostream &OS, const RecordKeeper &RK) {
  OS << "------------- Classes -----------------\n";
