...
bool isUTF8(llvh::StringRef S, size_t *ErrOffset = nullptr);
...
std::string fixUTF8(llvh::StringRef S);

class Array;
class ObjectKey;
class Value;
template <typename T> Value toJSON(const llvh::Optional<T> &Opt);

/// An Object is a JSON object, which maps strings to heterogenous JSON values.
/// It simulates DenseMap<ObjectKey, Value>. ObjectKey is a maybe-owned string.
class Object {
  using Storage = DenseMap<ObjectKey, Value, llvh::DenseMapInfo<StringRef>>;
  Storage M;
...
public:
  ...
  llvh::Optional<std::nullptr_t> getNull(StringRef K) const;
  llvh::Optional<bool> getBoolean(StringRef K) const;
  llvh::Optional<double> getNumber(StringRef K) const;
  llvh::Optional<int64_t> getInteger(StringRef K) const;
  llvh::Optional<llvh::StringRef> getString(StringRef K) const;
...
public:
  ...
  llvh::Optional<std::nullptr_t> getAsNull() const {
    if (LLVM_LIKELY(Type == T_Null))
      return nullptr;
    return llvh::None;
  }
  llvh::Optional<bool> getAsBoolean() const {
    if (LLVM_LIKELY(Type == T_Boolean))
      return as<bool>();
    return llvh::None;
  }
  llvh::Optional<double> getAsNumber() const {
  ...
  llvh::Optional<int64_t> getAsInteger() const {
    if (LLVM_LIKELY(Type == T_Integer))
      return as<int64_t>();
    if (LLVM_LIKELY(Type == T_Double)) {
      double D = as<double>();
  ...
  llvh::Optional<llvh::StringRef> getAsString() const {
    if (Type == T_String)
      return llvh::StringRef(as<std::string>());
    if (LLVM_LIKELY(Type == T_StringRef))
      return as<llvh::StringRef>();
    return llvh::None;
  }
...
private:
  ...
  mutable llvh::AlignedCharArrayUnion<bool, double, int64_t, llvh::StringRef,
...
public:
  ...
  ObjectKey(llvh::StringRef S) : Data(S) {
    if (LLVM_UNLIKELY(!isUTF8(Data))) {
      assert(false && "Invalid UTF-8 in value used as JSON");
      *this = ObjectKey(fixUTF8(S));
    }
  }
  ObjectKey(const llvh::SmallVectorImpl<char> &V)
      : ObjectKey(std::string(V.begin(), V.end())) {}
  ObjectKey(const llvh::formatv_object_base &V) : ObjectKey(V.str()) {}
  ...
  operator llvh::StringRef() const { return Data; }
...
inline bool operator==(const ObjectKey &L, const ObjectKey &R) {
  return llvh::StringRef(L) == llvh::StringRef(R);
}
inline bool operator!=(const ObjectKey &L, const ObjectKey &R) {
  return !(L == R);
}
...
template <typename T> bool fromJSON(const Value &E, llvh::Optional<T> &Out) {
  if (E.getAsNull()) {
    Out = llvh::None;
    return true;
  }
...
bool fromJSON(const Value &E, std::map<std::string, T> &Out) {
  if (auto *O = E.getAsObject()) {
    Out.clear();
    for (const auto &KV : *O)
      if (!fromJSON(KV.second, Out[llvh::StringRef(KV.first)]))
        return false;
    return true;
  }
  return false;
...
public:
  ...
  template <typename T> bool map(StringRef Prop, llvh::Optional<T> &Out) {
    assert(*this && "Must check this is an object before calling map()");
    if (const Value *E = O->get(Prop))
      return fromJSON(*E, Out);
    Out = llvh::None;
...
llvh::Expected<Value> parse(llvh::StringRef JSON);

class ParseError : public llvh::ErrorInfo<ParseError> {
  const char *Msg;
...
public:
  ...
  void log(llvh::raw_ostream &OS) const override {
    OS << llvh::formatv("[{0}:{1}, byte={2}]: {3}", Line, Column, Offset, Msg);
  }
  std::error_code convertToErrorCode() const override {
    return llvh::inconvertibleErrorCode();
  }
};
...
class OStream {
 public:
  using Block = llvh::function_ref<void()>;
  // If IndentSize is nonzero, output is pretty-printed.
  explicit OStream(llvh::raw_ostream &OS, unsigned IndentSize = 0)
  ...
  void attribute(llvh::StringRef Key, const Value& Contents) {
    attributeImpl(Key, [&] { value(Contents); });
  }
  /// Emit an attribute whose value is an array with elements from the Block.
  void attributeArray(llvh::StringRef Key, Block Contents) {
    attributeImpl(Key, [&] { array(Contents); });
  }
  /// Emit an attribute whose value is an object with attributes from the Block.
  void attributeObject(llvh::StringRef Key, Block Contents) {
  ...
  void objectBegin();
  void objectEnd();
  void attributeBegin(llvh::StringRef Key);
  void attributeEnd();

 private:
  void attributeImpl(llvh::StringRef Key, Block Contents) {
    attributeBegin(Key);
    Contents();
    attributeEnd();
  }
  ...
  llvh::SmallVector<State, 16> Stack; // Never empty.
  llvh::raw_ostream &OS;
  unsigned IndentSize;
  unsigned Indent = 0;
};
...
inline llvh::raw_ostream &operator<<(llvh::raw_ostream &OS, const Value &V) {
  OStream(OS).value(V);
  return OS;
}
} // namespace json
...
template <> struct format_provider<llvh::json::Value> {
  static void format(const llvh::json::Value &, raw_ostream &, StringRef);
};
} // namespace llvh
