/* @nolint */var initPromise = (function () {
  ...
  class Promise {
    ...
    static allKeyed(promisesObject) {
      if(typeof promisesObject !== 'object' || promisesObject === null) {
        var C = this;
        var capability = newPromiseCapability(C);
        capability.reject(new TypeError('Argument of Promise.allKeyed is not a non-null object'));
        return capability.promise;
      }
      var keys = Reflect.ownKeys(promisesObject);
      return this.all(
        keys.map(key => promisesObject[key])
      ).then(results => keys.reduce((acc, key, index) => {
    ...
    static allSettledKeyed(promisesObject) {
      if(typeof promisesObject !== 'object' || promisesObject === null) {
        var C = this;
        var capability = newPromiseCapability(C);
        capability.reject(new TypeError('Argument of Promise.allSettledKeyed is not a non-null object'));
        return capability.promise;
      }
      var keys = Reflect.ownKeys(promisesObject);
