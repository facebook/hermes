/* @nolint */var initPromise = (function () {
  ...
  class Promise {
    ...
    static allKeyed(promisesObject) {
      ...
      function decrementAndResolve(){
        if (--remainingElementsCount === 0) {
          capResolve.call(undefined, values);
        }
      }
      try {
    ...
    static allSettledKeyed(promisesObject) {
      ...
      var remainingElementsCount = 1;
      var values = Object.create(null);
      function insetToValues(key, value) {
          ...
          ((alreadyCalled, itemKey) => {
            promiseResolve.call(C, promisesObject[itemKey]).then(
              (value) => {
                if (alreadyCalled.value) return;
                alreadyCalled.value = true;
                insetToValues(itemKey, { status: 'fulfilled', value: value });
                decrementAndResolve();
              },
              (error) => {
                if (alreadyCalled.value) return;
                alreadyCalled.value = true;
                insetToValues(itemKey, { status: 'rejected', reason: error });
                decrementAndResolve();
              }
            );
          })({value: false}, key);
