...
bool canBeNaN(Value *value) {
  ...
  if (auto *literalNumber = dyn_cast<LiteralNumber>(value)) {
...
Value *reduceAsNumber(AsNumberInst *asNumber) {
  ...
  if (auto *lit = dyn_cast<Literal>(op)) {
...
Value *simplifyBinOp(BinaryOperatorInst *binary) {
  ...
  auto *litLhs = dyn_cast<Literal>(lhs);
  auto *litRhs = dyn_cast<Literal>(rhs);
  if (litLhs && litRhs) {
  ...
  switch (kind) {
    ...
    case OpKind::OrKind: { // |
      ...
      if (isa<LiteralNumber>(lhs) &&
          cast<LiteralNumber>(lhs)->getValue() == 0) {
        nonzeroOp = rhs;
      } else if (
          isa<LiteralNumber>(rhs) &&
...
Value *simplifyCondBranchInst(CondBranchInst *CBI) {
  ...
  // COND_BR %0, %BB2, %BB1
  //
  if (auto *U = dyn_cast<UnaryOperatorInst>(cond)) {
...
Value *getKnownReturnValue(Function *F, CallInst *callSite) {
  ...
  if (auto *P = dyn_cast<Parameter>(v)) {
...
Value *simplifyCallInst(CallInst *CI) {
  if (!CI->hasUsers() || isa<ConstructInst>(CI))
    return nullptr;
...
/// Try to simplify the instruction \p I. Return a simplified value that can be
/// a new instruction or some other value like a literal or a parameter, or null
/// if the instruction was not simplified. If a new value is returned then the
/// original instruction can be removed.
...
bool InstSimplify::runOnFunction(Function *F) {
  ...
  for (auto &blockIter : *F) {
    ...
    for (auto instIter = BB->begin(), e = BB->end(); instIter != e;) {
      ...
      Value *newVal = simplifyInstruction(II);
      if (!newVal)
        continue;
