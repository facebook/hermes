...
SwitchImmInst::SwitchImmInst(
  ...
  assert(minValue->isInt32Representible() && "minValue must be int32_t");
  ...
  for (size_t i = 0, e = values.size(); i < e; ++i) {
    // TODO: technically uint32_t should also be supported, but not 100% sure
    // about how clean that is for now.
    assert(values[i]->isInt32Representible() && "case value must be int32_t");
    pushOperand(values[i]);
    pushOperand(blocks[i]);
  }
}
