...
OptValue<HiddenClass::PropertyPos> HiddenClass::findProperty(
  ...
  desc = DictPropertyMap::getDescriptorPair(self->propertyMap_, *found)->second;
  return *found;
}

bool HiddenClass::debugIsPropertyDefined(HiddenClass *self, SymbolID name) {
  do {
    // If we happen to have a property map, use it.
    if (self->propertyMap_)
      return DictPropertyMap::find(self->propertyMap_, name).hasValue();
...
Handle<HiddenClass> HiddenClass::updateProperty(
  ...
  if (LLVM_UNLIKELY(selfHandle->flags_.dictionaryMode)) {
    ...
    DictPropertyMap::getDescriptorPair(selfHandle->propertyMap_, pos)
  ...
  childHandle->family_.set(selfHandle->family_, &runtime->getHeap());
  ...
  childHandle->propertyMap_.set(selfHandle->propertyMap_, &runtime->getHeap());
  selfHandle->propertyMap_ = nullptr;
...
Handle<HiddenClass> HiddenClass::makeAllNonConfigurable(
  ...
  DictPropertyMap::forEachProperty(
      ...
      [runtime, &curHandle](SymbolID id, NamedPropertyDescriptor desc) {
        ...
        auto found = DictPropertyMap::find(curHandle->propertyMap_, id);
...
void HiddenClass::initializeMissingPropertyMap(
  ...
  for (auto *cur = *selfHandle; cur->numProperties_ > 0; cur = cur->parent_) {
...
void HiddenClass::stealPropertyMapFromParent(
  ...
  self->propertyMap_.set(self->parent_->propertyMap_, &runtime->getHeap());
  self->parent_->propertyMap_ = nullptr;
  ...
  if (LLVM_LIKELY(!self->propertyFlags_.flagsTransition)) {
    ...
    assert(
        self->numProperties_ - 1 == self->propertyMap_->size() &&
    ...
    assert(
        self->numProperties_ == self->propertyMap_->size() &&
        "propertyMap->size() must match HiddenClass::numProperties in "
        "flag update transition");

    auto pos = DictPropertyMap::find(self->propertyMap_, self->symbolID_);
    assert(pos && "property must exist in flag update transition");
    auto tmpFlags = self->propertyFlags_;
    tmpFlags.flagsTransition = 0;
    DictPropertyMap::getDescriptorPair(self->propertyMap_, *pos)->second.flags =
        tmpFlags;
  }
}
