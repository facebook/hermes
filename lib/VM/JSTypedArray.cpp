...
ExecutionStatus JSTypedArrayBase::validateTypedArray(
  ...
  if (checkAttached && !self->attached()) {
    return runtime->raiseTypeError(
...
ExecutionStatus JSTypedArrayBase::setToCopyOfTypedArray(
  ...
  assert(
      dst->getBuffer()->getDataBlock() != src->getBuffer()->getDataBlock() &&
...
JSTypedArray<T, C>::JSTypedArray(
    ...
    HiddenClass *clazz,
    JSObjectPropStorage *propStorage)
    : JSTypedArrayBase(runtime, &vt.base.base, proto, clazz, propStorage) {
  byteWidth_ = sizeof(T);
}
...
CallResult<bool> JSTypedArray<T, C>::_setOwnIndexedImpl(
  ...
  if (LLVM_UNLIKELY(!typedArrayHandle->attached())) {
    return runtime->raiseTypeError(
        "Cannot set a value into a detached ArrayBuffer");
  }
  if (LLVM_LIKELY(index < typedArrayHandle->getLength())) {
    typedArrayHandle->template at<T>(index) = JSTypedArray<T, C>::toDestType(x);
  }
  return true;
}
