...
CallResult<HermesValue> stringRaw(void *, Runtime &runtime, NativeArgs args) {
  ...
  MutableHandle<> nextIndex{runtime, HermesValue::encodeTrustedNumberValue(0)};
  ...
  for (;; marker.flush()) {
    ...
    nextIndex = HermesValue::encodeTrustedNumberValue(
...
static CallResult<HermesValue> stringIndexOf(
  ...
  return stringDirectedIndexOf(
      ...
      string,
      searchValue,
      runtime.makeHandle(HermesValue::encodeTrustedNumberValue(fromIndex)),
      false,
      false);
}
