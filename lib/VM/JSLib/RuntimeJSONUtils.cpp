...
ExecutionStatus JSONStringifyer::initializeReplacer(Handle<> replacer) {
  ...
  for (uint32_t i = 0, e = arrayProperties->getEndIndex(); i < e; ++i) {
    auto index = arrayProperties->at(i);
    if (index.isNumber()) {
  ...
  for (uint32_t index : indexes) {
    ...
    for (uint32_t i = 0; i < len; ++i) {
      if (propertyList_->at(i).getString()->equals(tmpHandle_->getString())) {
        exists = true;
        break;
      }
    }
...
ExecutionStatus JSONStringifyer::initializeSpace(Handle<> space) {
  ...
  if (vmisa<JSNumber>(*tmpHandle_) || vmisa<JSString>(*tmpHandle_)) {
    tmpHandle_ = PrimitiveBox::getPrimitiveValue(vmcast<JSObject>(*tmpHandle_));
  }
...
CallResult<bool> JSONStringifyer::operationStr(HermesValue key) {
  ...
  if (vmisa<PrimitiveBox>(*operationStrValue_)) {
    operationStrValue_ =
        PrimitiveBox::getPrimitiveValue(vmcast<JSObject>(*operationStrValue_));
  }
...
ExecutionStatus JSONStringifyer::operationJO() {
  ...
  for (uint32_t index = 0, len = operationJOK_->getEndIndex(); index < len;
    ...
    tmpHandle_ = operationJOK_->at(index);
