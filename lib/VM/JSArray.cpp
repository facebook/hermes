...
bool ArrayImpl::_haveOwnIndexedImpl(
  ...
  if (index >= self->beginIndex_ && index < self->endIndex_)
    return !self->indexedStorage_->at(index - self->beginIndex_).isEmpty();
...
OptValue<PropertyFlags> ArrayImpl::_getOwnIndexedPropertyFlagsImpl(
  ...
  // Check whether the index is within the storage.
  if (index >= self->beginIndex_ && index < self->endIndex_ &&
      !self->indexedStorage_->at(index - self->beginIndex_).isEmpty()) {
...
HermesValue
ArrayImpl::_getOwnIndexedImpl(JSObject *selfObj, Runtime *, uint32_t index) {
  return vmcast<ArrayImpl>(selfObj)->at(index);
}
...
ExecutionStatus ArrayImpl::setStorageEndIndex(
  ...
  if (LLVM_UNLIKELY(!self->indexedStorage_)) {
    ...
    selfHandle->indexedStorage_.set(newStorage.get(), &runtime->getHeap());
  ...
  auto indexedStorage = createPseudoHandle(selfHandle->indexedStorage_.get());
  ...
  if (newLength < beginIndex) {
    ...
    return ExecutionStatus::RETURNED;
  } else if (newLength - beginIndex <= self->indexedStorage_->capacity()) {
    selfHandle->endIndex_ = newLength;
  ...
  selfHandle->indexedStorage_.set(
      indexedStorageHandle.get(), &runtime->getHeap());
  return ExecutionStatus::RETURNED;
}
...
CallResult<bool> ArrayImpl::_setOwnIndexedImpl(
  ...
  if (LLVM_LIKELY(index >= beginIndex && index < endIndex)) {
    self->indexedStorage_->at(index - beginIndex)
  ...
  if (LLVM_UNLIKELY(!self->indexedStorage_)) {
    ...
    self->indexedStorage_.set(newStorage.get(), &runtime->getHeap());
    self->beginIndex_ = index;
    self->endIndex_ = index + 1;
    self->indexedStorage_->at(0).set(value.get(), &runtime->getHeap());
    return true;
  }

  auto indexedStorage = createPseudoHandle(self->indexedStorage_.get());
  ...
  self->indexedStorage_.set(indexedStorageHandle.get(), &runtime->getHeap());
  return true;
}
...
bool ArrayImpl::_deleteOwnIndexedImpl(
  ...
  if (index >= self->beginIndex_ && index < self->endIndex_) {
    auto &elem = self->indexedStorage_->at(index - self->beginIndex_);
...
bool ArrayImpl::_checkAllOwnIndexedImpl(
  ...
  for (uint32_t i = 0, e = self->endIndex_ - self->beginIndex_; i != e; ++i) {
    if (!self->indexedStorage_->at(i).isEmpty())
      return false;
  }
  return true;
}
...
CallResult<bool> JSArray::setLength(
  ...
  if (LLVM_UNLIKELY(selfHandle->flags_.sealed)) {
    ...
    auto *self = selfHandle.get();
    auto range = _getOwnIndexedRangeImpl(self);
    ...
    for (; highestLen > lowestScanLen; --highestLen) {
      if (!self->unsafeAt(highestLen - 1).isEmpty()) {
