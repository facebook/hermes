...
import static com.facebook.hermes.intl.IPlatformDateTimeFormatter.DateStyle.*;
...
public class PlatformDateTimeFormatterICU implements IPlatformDateTimeFormatter {
  ...
  @RequiresApi(api = Build.VERSION_CODES.N)
  private static String getPatternForStyle(ILocaleObject<?> resolvedLocaleObject,
                                           DateStyle dateStyle,
                                           TimeStyle timeStyle) throws JSRangeErrorException {
    if(dateStyle == UNDEFINED) {
      return ((SimpleDateFormat)DateFormat.getTimeInstance (toICUTimeStyle(timeStyle), (ULocale) resolvedLocaleObject.getLocale())).toLocalizedPattern();
    } else if (timeStyle == TimeStyle.UNDEFINED) {
      return ((SimpleDateFormat)DateFormat.getDateInstance (toICUDateStyle(dateStyle), (ULocale) resolvedLocaleObject.getLocale())).toLocalizedPattern();
    } else {
      return ((SimpleDateFormat)DateFormat.getDateTimeInstance (toICUDateStyle(dateStyle),toICUTimeStyle(timeStyle),  (ULocale) resolvedLocaleObject.getLocale())).toLocalizedPattern();
    }
  }
  ...
  private static String getSkeleton(
      ...
      DateStyle dateStyle,
      TimeStyle timeStyle,
      Object hour12) throws JSRangeErrorException {

    StringBuilder skeletonBuffer = new StringBuilder();

    // For reference on patterns/skleleton: https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html
    ...
    if (dateStyle != UNDEFINED || timeStyle != TimeStyle.UNDEFINED) {
      ...
      if (hourCycle == HourCycle.H11 || hourCycle == HourCycle.H12) skeletonBuffer.append(hour.getSkeleonSymbol12());
