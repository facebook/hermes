...
public class Collator {
    ...
    private boolean containsString(String[] list, final String testValue) {
...
//    }
        ...
        if (options.containsKey(Constants.USAGE)) {
            ...
            if (containsString(Constants.COLLATOR_USAGE_POSSIBLE_VALUES, optionUsage)) {
                ...
                throw new JSRangeErrorException(String.format("Invalid value '%s' for option %s", optionUsage, Constants.USAGE));
            }
        }
        ...
        if (options.containsKey(Constants.SENSITIVITY)) {
            ...
            if (containsString(Constants.SENSITIVITY_POSSIBLE_VALUES, optionSensitivity)) {
                ...
                throw new JSRangeErrorException(String.format("Invalid value '%s' for option %s", optionSensitivity, Constants.SENSITIVITY));
            }
        }
        ...
        if (options.containsKey(Constants.CASEFIRST)) {
            ...
            if (containsString(Constants.CASEFIRST_POSSIBLE_VALUES, optionCaseFirst)) {
                resolvedCaseFirst = optionCaseFirst;
                resolvedCaseFirstSet = true;
            } else {
                throw new JSRangeErrorException(String.format("Invalid value '%s' for option %s", optionCaseFirst, Constants.CASEFIRST));
            }
        } else {
            // TODO: Check extensions in localeId
        }
    ...
    public Collator(List<String> locales, Map<String, Object> options)
        ...
        if(resolvedLocaleObject == null) {
          // TODO :: We need to try harder to find a reasonable match before falling back to default ..
          resolvedLocaleObject = LocaleObject.constructDefault();
        }
