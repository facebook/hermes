...
static void tsfnDispatch(void *tsfn_data) {
  ...
  if (has_more && env->host_ != nullptr) {
    ...
    env->host_->post_task(
        env->host_->data, tsfn, tsfnDispatch);
  }
}
...
napi_status NAPI_CDECL napi_create_threadsafe_function(
  ...
  RETURN_STATUS_IF_FALSE(
      env, initial_thread_count > 0, napi_invalid_arg);

  // A host must be provided for thread-safe functions.
  RETURN_STATUS_IF_FALSE(
      env, env->host_ != nullptr, napi_generic_failure);
  RETURN_STATUS_IF_FALSE(
  ...
  if (func == nullptr) {
    RETURN_STATUS_IF_FALSE(
        env, call_js_cb != nullptr, napi_invalid_arg);
  } else {
    auto *phv = reinterpret_cast<const PinnedHermesValue *>(func);
    RETURN_STATUS_IF_FALSE(
        env,
        phv->isObject() && vmisa<Callable>(*phv),
        napi_invalid_arg);
  }
...
napi_status NAPI_CDECL napi_call_threadsafe_function(
  ...
  if (func->dispatch_pending.compare_exchange_strong(
    ...
    func->env->host_->post_task(
        func->env->host_->data, func, tsfnDispatch);
  }
