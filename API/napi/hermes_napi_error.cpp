...
static napi_status createErrorWithCode(
  ...
  auto setMsgRes =
      JSError::setMessage(errorHandle, runtime, msgHandle);
...
static napi_status throwErrorWithMessage(
  ...
  // Set the message property.
  auto setMsgRes =
      JSError::setMessage(errorHandle, runtime, lv.msgStr);
  if (LLVM_UNLIKELY(setMsgRes == ExecutionStatus::EXCEPTION)) {
    return captureRuntimeException(env, napi_generic_failure);
  }
...
napi_status NAPI_CDECL napi_is_exception_pending(
    napi_env env,
    bool *result) {
...
napi_status NAPI_CDECL napi_create_error(
  ...
  if (code != nullptr) {
    auto *codePhv =
        reinterpret_cast<hermes::vm::PinnedHermesValue *>(code);
    RETURN_STATUS_IF_FALSE(
        env, codePhv->isString(), napi_string_expected);
  }
...
napi_status NAPI_CDECL napi_is_error(
    napi_env env,
    napi_value value,
    bool *result) {
  ...
  // JSError instance.
  *result = hv.isObject() &&
      hermes::vm::vmisa<hermes::vm::JSError>(hv);
  return napi_clear_last_error(env);
}
