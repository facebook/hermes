...
napi_status NAPI_CDECL napi_create_arraybuffer(
  ...
  auto status =
      JSArrayBuffer::createDataBlock(runtime, lv.ab, byte_length);
...
napi_status NAPI_CDECL napi_create_external_arraybuffer(
  ...
  if (finalize_cb) {
    ...
    ctx = std::shared_ptr<void>(
        external_data,
        [env, envAlive, finalize_cb, finalize_hint](void *data) {
          if (*envAlive) {
...
napi_status NAPI_CDECL napi_detach_arraybuffer(
    napi_env env,
    napi_value arraybuffer) {
  NAPI_PREAMBLE(env);
  CHECK_ARG(env, arraybuffer);
  ...
  JSArrayBuffer::detach(
      runtime, Handle<JSArrayBuffer>::vmcast(handle));
