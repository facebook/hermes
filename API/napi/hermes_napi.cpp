...
napi_env__::napi_env__(
  ...
  // microtask batches).
  runtime.addDrainJobsCallback([this]() {
...
napi_status NAPI_CDECL napi_get_last_error_info(
  ...
  if (env->last_error.error_code <= lastStatus) {
    env->last_error.error_message =
        errorMessages[env->last_error.error_code];
  } else {
...
napi_status NAPI_CDECL napi_get_uv_event_loop(
    node_api_basic_env env,
...
napi_status NAPI_CDECL napi_get_instance_data(
    node_api_basic_env env,
    void **data) {
  CHECK_ENV(env);
  CHECK_ARG(env, data);
...
napi_status NAPI_CDECL napi_run_script(
    napi_env env,
    napi_value script,
    napi_value *result) {
  NAPI_PREAMBLE(env);
...
napi_status hermes_run_script(
  ...
  auto bytecodeErr =
      hermes::hbc::createBCProviderFromSrc(std::move(buffer), sourceURL, {}, compileFlags);
...
napi_status hermes_run_bytecode(
  ...
  // Parse bytecode.
  auto bcResult = hermes::hbc::BCProviderFromBuffer::
      createBCProviderFromBuffer(std::move(buffer));
  if (!bcResult.first) {
...
napi_status hermes_napi_load_module(
    napi_env env,
    const char *path,
    napi_value *result) {
  NAPI_PREAMBLE(env);
  ...
  auto initFunc = reinterpret_cast<InitFunc>(
  ...
  if (!initFunc) {
    dlclose(handle);
    (void)runtime.raiseTypeError(
        TwineChar16("Module has no 'napi_register_module_v1' "
  ...
  napi_value exportsValue =
      env->addToCurrentScope(exports.getHermesValue());
