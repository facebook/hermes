...
static napi_status createBufferUint8Array(
  ...
  if (data != nullptr) {
    *data = vmcast<JSArrayBuffer>(lv.ab.getHermesValue())
                ->getDataBlock();
  }
...
napi_status NAPI_CDECL napi_create_buffer_copy(
  ...
  auto status =
      createBufferUint8Array(env, length, &bufData, result);
  if (status != napi_ok) {
    return status;
  }
...
napi_is_buffer(napi_env env, napi_value value, bool *result) {
  ...
  *result = phv->isObject() &&
      hermes::vm::vmisa<hermes::vm::Uint8Array>(*phv);
  return napi_clear_last_error(env);
}
...
napi_status NAPI_CDECL napi_get_buffer_info(
  ...
  RETURN_STATUS_IF_FALSE(
      env,
      phv->isObject() && vmisa<Uint8Array>(*phv),
      napi_invalid_arg);
...
napi_status NAPI_CDECL napi_create_external_buffer(
  ...
  if (finalize_cb) {
    ...
    ctx = std::shared_ptr<void>(
        data,
        [env, envAlive, finalize_cb, finalize_hint](void *d) {
          if (*envAlive) {
...
napi_status NAPI_CDECL node_api_create_buffer_from_arraybuffer(
  ...
  auto *abPhv =
      reinterpret_cast<PinnedHermesValue *>(arraybuffer);
  RETURN_STATUS_IF_FALSE(
      env,
      abPhv->isObject() && vmisa<JSArrayBuffer>(*abPhv),
      napi_invalid_arg);
  ...
  RETURN_STATUS_IF_FALSE(
      env,
      byte_offset + byte_length <= ab->size(),
      napi_invalid_arg);
