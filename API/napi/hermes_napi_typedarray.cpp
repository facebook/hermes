...
napi_status NAPI_CDECL napi_create_typedarray(
  ...
  if (elemSize > 1) {
    RETURN_STATUS_IF_FALSE(
        env, byte_offset % elemSize == 0, napi_invalid_arg);
  }
  ...
  RETURN_STATUS_IF_FALSE(
      env,
      byte_offset + byteLength <= ab->size(),
      napi_invalid_arg);
  ...
  auto status = createTypedArrayForType(
      type, runtime, ab, byte_offset, length, lv.ta);
...
napi_status NAPI_CDECL napi_get_typedarray_info(
  ...
  RETURN_STATUS_IF_FALSE(
      env,
      phv->isObject() && vmisa<JSTypedArrayBase>(*phv),
      napi_invalid_arg);
  ...
  if (type != nullptr) {
    ...
    if (!cellKindToTypedArrayType(
            static_cast<GCCell *>(phv->getObject())->getKind(),
            &taType)) {
...
napi_is_typedarray(napi_env env, napi_value value, bool *result) {
  ...
  *result = phv->isObject() &&
      hermes::vm::vmisa<hermes::vm::JSTypedArrayBase>(*phv);
  return napi_clear_last_error(env);
}
