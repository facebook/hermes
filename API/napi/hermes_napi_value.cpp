...
napi_get_boolean(napi_env env, bool value, napi_value *result) {
  ...
  CHECK_ARG(env, result);
  *result = env->addToCurrentScope(
      hermes::vm::HermesValue::encodeBoolValue(value));
  return napi_clear_last_error(env);
}
...
napi_status NAPI_CDECL napi_get_global(napi_env env, napi_value *result) {
  ...
  CHECK_ARG(env, result);
  *result = env->addToCurrentScope(
      *hermes::vm::toPHV(&env->runtime.global_));
  return napi_clear_last_error(env);
}
...
napi_status NAPI_CDECL napi_typeof(
    napi_env env,
    napi_value value,
...
napi_status NAPI_CDECL napi_coerce_to_bool(
    napi_env env,
    napi_value value,
    napi_value *result) {
  NAPI_PREAMBLE(env);
...
napi_status NAPI_CDECL napi_coerce_to_number(
  ...
  if (LLVM_UNLIKELY(
          numRes == hermes::vm::ExecutionStatus::EXCEPTION)) {
...
napi_status NAPI_CDECL napi_coerce_to_object(
  ...
  if (LLVM_UNLIKELY(
          objRes == hermes::vm::ExecutionStatus::EXCEPTION)) {
    return captureRuntimeException(env, napi_pending_exception);
  }
...
napi_status NAPI_CDECL napi_strict_equals(
  ...
  auto *lhv =
      reinterpret_cast<hermes::vm::PinnedHermesValue *>(lhs);
  auto *rhv =
      reinterpret_cast<hermes::vm::PinnedHermesValue *>(rhs);
...
napi_status NAPI_CDECL napi_instanceof(
  ...
  *result = false;
  ...
  hermes::vm::GCScope gcScope(env->runtime);
  auto instRes = hermes::vm::instanceOfOperator_RJS(
      env->runtime, objHandle, ctorHandle);
  if (LLVM_UNLIKELY(
          instRes == hermes::vm::ExecutionStatus::EXCEPTION)) {
    return captureRuntimeException(env, napi_pending_exception);
  }
...
napi_status NAPI_CDECL napi_create_symbol(
  ...
  auto symbolRes =
      runtime.getIdentifierTable().createNotUniquedSymbol(
          runtime, lv.descStr);
  ...
  *result = env->addToCurrentScope(
      HermesValue::encodeSymbolValue(*symbolRes));
  return napi_clear_last_error(env);
}
...
napi_status NAPI_CDECL node_api_symbol_for(
  ...
  auto strRes = StringPrimitive::createEfficient(
      runtime,
      UTF8Ref(
          reinterpret_cast<const uint8_t *>(utf8description), length));
  ...
  auto symbolRes =
      runtime.getSymbolRegistry().getSymbolForKey(runtime, lv.key);
  if (LLVM_UNLIKELY(symbolRes == ExecutionStatus::EXCEPTION)) {
    return captureRuntimeException(env, napi_generic_failure);
  }
...
napi_status NAPI_CDECL napi_create_date(
    napi_env env,
    double time,
    napi_value *result) {
  CHECK_ENV(env);
  ...
  auto dateObj = JSDate::create(
      runtime,
      time,
      Handle<JSObject>::vmcast(&runtime.datePrototype));

  *result = env->addToCurrentScope(dateObj.getHermesValue());
  return napi_clear_last_error(env);
}

napi_status NAPI_CDECL napi_is_date(
    napi_env env,
    napi_value value,
    bool *is_date) {
  CHECK_ENV(env);
...
napi_status NAPI_CDECL napi_get_date_value(
    napi_env env,
    napi_value value,
    double *result) {
  CHECK_ENV(env);
