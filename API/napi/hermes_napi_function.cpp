...
napiFunctionTrampoline(void *context, hermes::vm::Runtime &runtime) {
  ...
  if (result != nullptr) {
    ...
    auto *phv =
        reinterpret_cast<PinnedHermesValue *>(result);
...
napi_status NAPI_CDECL napi_create_function(
  ...
  if (utf8name != nullptr) {
    size_t nameLen = (length == NAPI_AUTO_LENGTH)
        ? std::strlen(utf8name)
        : length;
    if (nameLen > 0) {
...
napi_status NAPI_CDECL napi_call_function(
  ...
  RETURN_STATUS_IF_FALSE(
      env,
      phvFunc->isObject() && vmisa<Callable>(*phvFunc),
      napi_invalid_arg);
  ...
  for (size_t i = 0; i < argc; ++i) {
    auto *phvArg =
        reinterpret_cast<const PinnedHermesValue *>(argv[i]);
    newFrame->getArgRef(i) = *phvArg;
  }

  // Execute the call.
  auto callRes = Callable::call(
      Handle<Callable>::vmcast(phvFunc), runtime);
...
napi_status NAPI_CDECL napi_new_instance(
  ...
  auto *phvCtor =
      reinterpret_cast<const PinnedHermesValue *>(constructor);
  RETURN_STATUS_IF_FALSE(
      env,
      phvCtor->isObject() && vmisa<Callable>(*phvCtor),
      napi_invalid_arg);
  ...
  // Execute the call.
  auto callRes = Callable::call(
      Handle<Callable>::vmcast(phvCtor), runtime);
  if (LLVM_UNLIKELY(callRes == ExecutionStatus::EXCEPTION)) {
    return captureRuntimeException(env, napi_pending_exception);
  }
...
hermes::vm::CallResult<hermes::vm::HermesValue>
napiConstructorTrampoline(void *context, hermes::vm::Runtime &runtime) {
  using namespace hermes::vm;
  ...
  if (!args.getNewTarget().isUndefined()) {
    ...
    auto parentHandle =
        runtime.makeHandle(std::move(*protoRes));
    ...
    napi_value thisSlot =
        env->addToCurrentScope(thisObj.getHermesValue());
    cbinfo.thisArg =
        reinterpret_cast<PinnedHermesValue *>(thisSlot);
  } else {
  ...
  if (!args.getNewTarget().isUndefined()) {
    ...
    std::memcpy(
        &thisVal, cbinfo.thisArg, sizeof(HermesValue));
    napi_close_handle_scope(env, scope);
    return thisVal;
  }
...
napi_status NAPI_CDECL napi_define_class(
  ...
  if (utf8name != nullptr) {
    ...
    if (nameLen > 0) {
      ...
      auto symRes =
          runtime.getIdentifierTable()
              .getSymbolHandleFromPrimitive(
                  runtime,
                  PseudoHandle<StringPrimitive>::create(
                      vmcast<StringPrimitive>(*strRes)));
      if (LLVM_UNLIKELY(symRes == ExecutionStatus::EXCEPTION)) {
  ...
  auto ctorPH = NativeConstructor::create(
      ...
      napiConstructorTrampoline,
      0 /* paramCount */);
  lv.ctor.castAndSetHermesValue<Callable>(
      ctorPH.getHermesValue());

  gcScope.flushToMarker(marker);

  // Create the prototype object.
  auto protoObj = JSObject::create(runtime);
  lv.proto.castAndSetHermesValue<JSObject>(
      protoObj.getHermesValue());
  ...
  for (size_t i = 0; i < property_count; ++i) {
    ...
    if (p->utf8name != nullptr) {
      ...
      auto internStatus =
          internUtf8AsSymbol(env, p->utf8name, symRes);
      ...
      if (namePhv->isString()) {
        ...
        if (LLVM_UNLIKELY(
                symRes == ExecutionStatus::EXCEPTION)) {
          return captureRuntimeException(
    ...
    if (p->getter != nullptr || p->setter != nullptr) {
      ...
      Handle<Callable> getterHandle =
          Runtime::makeNullHandle<Callable>();
      Handle<Callable> setterHandle =
          Runtime::makeNullHandle<Callable>();
      ...
      auto accessor = PropertyAccessor::create(
          runtime, getterHandle, setterHandle);
      auto accessorHandle =
      ...
      dpf.setEnumerable = 1;
      dpf.enumerable =
          (p->attributes & napi_enumerable) ? 1 : 0;
      dpf.setConfigurable = 1;
      dpf.configurable =
          (p->attributes & napi_configurable) ? 1 : 0;
      dpf.setGetter = 1;
      dpf.setSetter = 1;
      ...
      if (LLVM_UNLIKELY(
              propRes == ExecutionStatus::EXCEPTION)) {
        return captureRuntimeException(
            env, napi_pending_exception);
      }
      RETURN_STATUS_IF_FALSE(
          env, *propRes, napi_invalid_arg);
      ...
      dpf.setWritable = 1;
      dpf.writable =
          (p->attributes & napi_writable) ? 1 : 0;
      dpf.setConfigurable = 1;
      dpf.configurable =
          (p->attributes & napi_configurable) ? 1 : 0;
      dpf.setValue = 1;
      ...
      RETURN_STATUS_IF_FALSE(
          env, *propRes, napi_generic_failure);
    } else {
