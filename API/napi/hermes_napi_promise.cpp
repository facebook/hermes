...
void napi_env__::markDeferreds(
    hermes::vm::RootAcceptor &acceptor) {
...
napi_status NAPI_CDECL napi_create_promise(
  ...
  RETURN_STATUS_IF_FALSE(
      env,
      (*promiseCtorRes)->isObject() &&
          vmisa<Callable>(promiseCtorRes->get()),
      napi_generic_failure);

  lv.promiseCtor.castAndSetHermesValue<Callable>(
      promiseCtorRes->get());
  ...
  lv.executor.castAndSetHermesValue<Callable>(
      executorFunc.getHermesValue());

  // Call `new Promise(executor)` using the standard construct helper.
  auto constructRes = Callable::executeConstruct1(
      lv.promiseCtor,
      runtime,
      lv.executor);
...
static napi_status concludeDeferred(
  ...
  auto *resPhv =
      reinterpret_cast<const PinnedHermesValue *>(resolution);

  // Get the resolve or reject function from the deferred.
  PinnedHermesValue &funcPhv =
      isResolve ? deferred->resolve : deferred->reject;
  ...
  auto callRes = Callable::call(
      Handle<Callable>::vmcast(&funcPhv), runtime);
...
napi_status NAPI_CDECL napi_is_promise(
    napi_env env,
    napi_value value,
    bool *is_promise) {
  CHECK_ENV(env);
  ...
  *is_promise = false;
  ...
  auto instRes =
      instanceOfOperator_RJS(runtime, valHandle, ctorHandle);
