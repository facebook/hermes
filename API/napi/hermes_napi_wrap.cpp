...
static SymbolID napiDataSymbolID() {
  return Predefined::getSymbolID(
      Predefined::InternalPropertyNapiData);
}
...
static void finalizeNapiObjectData(GC &, NativeState *ns) {
  ...
  if (data->wrapFinalizeCb) {
    if (envAlive) {
      ...
      data->wrapFinalizeCb(
          nullptr, data->wrapData, data->wrapFinalizeHint);
    }
  }
...
napi_status getOrCreateNapiObjectData(
  ...
  auto *ns =
      NativeState::create(runtime, data, finalizeNapiObjectData);

  auto valHandle =
      runtime.makeHandle(HermesValue::encodeObjectValue(ns));
...
napi_status NAPI_CDECL napi_wrap(
  ...
  napiData->wrapFinalizeHint =
      result != nullptr ? nullptr : finalize_hint;
