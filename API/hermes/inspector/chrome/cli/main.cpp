...
static void runDebuggerLoop(
    std::shared_ptr<fbhermes::inspector_modern::chrome::CDPHandler> cdpHandler,
...
static void runScript(const std::string &scriptSource, const std::string &url) {
  ...
  auto adapter =
      std::make_unique<fbhermes::inspector_modern::SharedRuntimeAdapter>(
          runtime);
  auto cdpHandler = fbhermes::inspector_modern::chrome::CDPHandler::create(
      std::move(adapter), "hermes-chrome-debug-server");
  std::thread debuggerLoop(runDebuggerLoop, cdpHandler, scriptSource);
