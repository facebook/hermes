...
struct CDPBreakpoint {
  ...
  // IDs of registered breakpoints in Hermes
  std::vector<debugger::BreakpointID> hermesBreakpoints;
};
...
class CDPHandlerImpl : public message::RequestHandler,
 ...
 public:
  ...
  std::pair<unsigned int, CDPBreakpoint &> createCDPBreakpoint(
      CDPBreakpointDescription &&description,
      debugger::BreakpointID hermesBreakpointID =
          debugger::kInvalidBreakpoint) {
    ...
    if (hermesBreakpointID != debugger::kInvalidBreakpoint) {
      breakpoint.hermesBreakpoints.push_back(hermesBreakpointID);
    }
  ...
  std::optional<HermesBreakpointLocation> applyBreakpoint(
    ...
    breakpoint.hermesBreakpoints.push_back(hermesBreakpoint.value().id);
...
void CDPHandlerImpl::handle(const m::debugger::RemoveBreakpointRequest &req) {
  enqueueFunc([this, req]() {
    if (isVirtualBreakpointId(req.breakpointId)) {
      ...
      for (debugger::BreakpointID hermesBreakpoint :
           cdpBreakpoint->second.hermesBreakpoints) {
        getDebugger().deleteBreakpoint(hermesBreakpoint);
      }
...
void CDPHandlerImpl::handle(const m::debugger::SetBreakpointRequest &req) {
  enqueueFunc([this,
    ...
    auto [breakpointID, breakpoint] = createCDPBreakpoint(
        std::move(description), hermesBreakpoint.value().id);
