    ...
    def update(self, test):
        ...
        if self.progressBar:
            self.progressBar.update(
                float(self.completed) / self.numTests, test.getFullName()
            )

        shouldShow = (
            test.result.code.isFailure
        ...
        print(
            "%s: %s (%d of %d)"
            % (test.result.code.name, test_name, self.completed, self.numTests)
        )

        # Show the test failure output, if requested.
        if (
            test.result.code.isFailure and self.opts.showOutput
        ) or self.opts.showAllOutput:
...
def main_with_tmp(builtinParameters):
    ...
    parser.add_argument(
        "test_paths", nargs="*", help="Files or paths to include in the test suite"
    )
    ...
    opts = parser.parse_args(sys.argv[1:] + shlex.split(os.environ.get("LIT_OPTS", "")))
        ...
        if opts.maxIndividualTestTime != litConfig.maxIndividualTestTime:
            litConfig.note(
                (
                    ...
                    " seconds"
                ).format(litConfig.maxIndividualTestTime, opts.maxIndividualTestTime)
            )
    ...
    try:
        ...
        if max_procs_soft < desired_limit:
            ...
            litConfig.note(
                "raised the process limit from %d to %d"
    ...
    threads = (
        "single process" if (opts.numThreads == 1) else ("%d threads" % opts.numThreads)
    )
    ...
    for title, code in (
        ("Unexpected Passing Tests", lit.Test.XPASS),
        ...
        if (
            (lit.Test.XFAIL == code and not opts.show_xfail)
            or (lit.Test.UNSUPPORTED == code and not opts.show_unsupported)
            or (lit.Test.UNRESOLVED == code and (opts.maxFailures is not None))
        ):
            continue
    ...
    if opts.xunit_output_file:
        ...
        for suite_name, suite in by_suite.items():
            ...
            xunit_output_file.write(
                ' tests="'
                + str(suite["passes"] + suite["failures"] + suite["skipped"])
                + '"'
            )
