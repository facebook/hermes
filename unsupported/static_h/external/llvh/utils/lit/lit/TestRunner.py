...
def executeBuiltinDiff(cmd, cmd_shenv):
    ...
    def compareTwoTextFiles(filepaths, encoding):
        ...
        if ignore_all_space or ignore_space_change:
            ...
            ignoreAllSpaceOrSpaceChange = functools.partial(
                ignoreSpace, separator="" if ignore_all_space else " "
            )
    ...
    return ShellCommandResult(
        cmd, stdout.getvalue(), stderr.getvalue(), exitCode, False
    )
...
def _executeShCmd(cmd, shenv, results, timeoutHelper):
    ...
    if cmd.commands[0].args[0] == "echo" and len(cmd.commands) == 1:
        ...
        results.append(ShellCommandResult(cmd.commands[0], output, "", 0, False))
        return 0
    ...
    if cmd.commands[0].args[0] == "mkdir":
        if len(cmd.commands) != 1:
            raise InternalShellError(
                cmd.commands[0], "Unsupported: 'mkdir' " "cannot be part of a pipeline"
            )
    ...
    if cmd.commands[0].args[0] == "diff":
        if len(cmd.commands) != 1:
            raise InternalShellError(
                cmd.commands[0], "Unsupported: 'diff' " "cannot be part of a pipeline"
            )
    ...
    if cmd.commands[0].args[0] == "rm":
        if len(cmd.commands) != 1:
            raise InternalShellError(
                cmd.commands[0], "Unsupported: 'rm' " "cannot be part of a pipeline"
            )
    ...
    if cmd.commands[0].args[0] == ":":
        if len(cmd.commands) != 1:
            raise InternalShellError(
                cmd.commands[0], "Unsupported: ':' " "cannot be part of a pipeline"
            )
...
class IntegratedTestKeywordParser(object):
    ...
    def _handleRequiresAny(line_number, line, output):
        ...
        IntegratedTestKeywordParser._handleBooleanExpr(line_number, expression, output)
        return output
...
def parseIntegratedTestScript(test, additional_parsers=[], require_script=True):
    ...
    builtin_parsers = [
        ...
        IntegratedTestKeywordParser(
            "XFAIL:", ParserKind.BOOLEAN_EXPR, initial_value=test.xfails
        ),
        IntegratedTestKeywordParser(
            "REQUIRES:", ParserKind.BOOLEAN_EXPR, initial_value=test.requires
        ),
        IntegratedTestKeywordParser(
            "REQUIRES-ANY:",
            ParserKind.CUSTOM,
        ...
        IntegratedTestKeywordParser(
            "UNSUPPORTED:", ParserKind.BOOLEAN_EXPR, initial_value=test.unsupported
        ),
    ...
    if script and script[-1][-1] == "\\":
        return lit.Test.Result(
            Test.UNRESOLVED, "Test has unterminated run lines (with '\\')"
        )
