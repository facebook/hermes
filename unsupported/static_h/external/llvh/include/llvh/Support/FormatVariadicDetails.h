...
public:
  ...
  void format(llvh::raw_ostream &S, StringRef Options) override {
...
public:
  ...
  void format(llvh::raw_ostream &S, StringRef Options) override { S << Item; }
};
...
public:
  ...
  typedef void (*Signature_format)(const Decayed &, llvh::raw_ostream &,
                                   StringRef);
  ...
  static bool const value =
      (sizeof(test<llvh::format_provider<Decayed>>(nullptr)) == 1);
};
...
public:
  ...
  static char test(typename std::enable_if<
                   std::is_same<decltype(std::declval<llvh::raw_ostream &>()
                                         << std::declval<U>()),
                                llvh::raw_ostream &>::value,
                   int *>::type);
...
build_format_adapter(T &&Item) {
  ...
  static_assert(
      !std::is_same<llvh::Error, typename std::remove_cv<T>::type>::value,
      "llvh::Error-by-value must be wrapped in fmt_consume() for formatv");
