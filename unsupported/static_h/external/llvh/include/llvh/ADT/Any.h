...
class Any {
  ...
  template <typename T> struct StorageImpl : public StorageBase {
    ...
    std::unique_ptr<StorageBase> clone() const override {
      return llvh::make_unique<StorageImpl<T>>(Value);
    }
...
public:
  ...
  template <
      ...
      typename std::enable_if<
          llvh::conjunction<
              llvh::negation<std::is_same<typename std::decay<T>::type, Any>>,
              ...
              llvh::negation<std::is_convertible<Any, typename std::decay<T>::type>>,
              std::is_copy_constructible<typename std::decay<T>::type>>::value,
          int>::type = 0>
  Any(T &&Value) {
    using U = typename std::decay<T>::type;
    Storage = llvh::make_unique<StorageImpl<U>>(std::forward<T>(Value));
  }
