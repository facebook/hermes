...
int main(int argc, char **argv) {
  ...
  if (!CompileOutput.empty()) {
    ...
    status = hermes_compile_to_bytecode(
        env, data, size, InputFilename.c_str(), &cflags,
    ...
    if (ec) {
      llvh::errs() << "Error: could not open output file: "
                    << CompileOutput << ": " << ec.message() << "\n";
  ...
  if (isBytecode) {
    status = hermes_run_bytecode(
        env, data, size, nullptr, nullptr, InputFilename.c_str(), nullptr,
        &result);
  } else {
    ...
    status = hermes_run_script(
        env, data, size, nullptr, nullptr,
        InputFilename.c_str(), &rflags, &result);
  }
