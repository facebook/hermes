(function (factory) {
  ...
  var skipChainExpression = (fn) => (node) => {
    if ((node == null ? void 0 : node.type) === "ChainExpression") {
...
Expected it to be ${EXPECTED_TYPE_VALUES}.`;
  ...
  function handleCommentsInDestructuringPattern({
    ...
    if (((enclosingNode == null ? void 0 : enclosingNode.type) === "ObjectPattern" || (enclosingNode == null ? void 0 : enclosingNode.type) === "ArrayPattern") && (followingNode == null ? void 0 : followingNode.type) === "TSTypeAnnotation") {
      if (precedingNode) {
        addTrailingComment(precedingNode, comment);
      } else {
        addDanglingComment(enclosingNode, comment);
...
` : "") + docBlock + (strippedText.startsWith("\n") ? "\n" : "\n\n") + strippedText;
  ...
  function printCallArguments(path, options2, print3) {
    ...
    if (isReactHookCallWithDepsArray(args)) {
      const parts = ["("];
      iterateCallArgumentsPath(path, (path2, index) => {
        parts.push(print3());
  ...
  function isReactHookCallWithDepsArray(args) {
    if (args.length === 2) {
      return isValidHookCallbackAndDepsFormat(
        args,
        /* baseIndex */
        0
      );
    ...
    if (args.length === 3) {
      return args[0].type === "Identifier" && isValidHookCallbackAndDepsFormat(
        args,
        /* baseIndex */
        1
      );
  ...
  function isValidHookCallbackAndDepsFormat(args, baseIndex) {
    ...
    return maybeArrowFunction.type === "ArrowFunctionExpression" && getFunctionParameters(maybeArrowFunction).length === 0 && maybeArrowFunction.body.type === "BlockStatement" && maybeDepsArray.type === "ArrayExpression" && !args.some((arg) => hasComment(arg));
  }
  ...
  function getImportAttributesKeyword(node, options2) {
    var _a, _b;
    if ((_a = node.extra) == null ? void 0 : _a.deprecatedAssertSyntax) {
      return "assert";
    }
    const textBetweenSourceAndAttributes = get_text_without_comments_default(
      options2,
      locEnd(node.source),
      ((_b = node.attributes) == null ? void 0 : _b[0]) ? locStart(node.attributes[0]) : locEnd(node)
    ).trimStart();
  ...
  function printTernaryOld(path, options2, print3) {
    ...
    if (isConditionalExpression && (isJsxElement(node[testNodePropertyNames[0]]) || isJsxElement(consequentNode) || isJsxElement(alternateNode) || conditionalExpressionChainContainsJsx(lastConditionalParent))) {
      ...
      const printBranch = (nodePropertyName) => options2.useTabs ? indent(print3(nodePropertyName)) : align(2, print3(nodePropertyName));
      const part = [
        line,
        "? ",
  ...
  function clean2(original, cloned) {
    const { type } = original;
