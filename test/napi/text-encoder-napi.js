...
var addon = loadNativeModule("ADDON_PATH");
var TextEncoderNApi = addon.TextEncoderNApi;

print('TextEncoderNApi');
// CHECK-LABEL: TextEncoderNApi

var encoder = new TextEncoderNApi();
print(Object.prototype.toString.call(encoder));
// CHECK-NEXT: [object TextEncoderNApi]

const desc = Object.getOwnPropertyDescriptor(TextEncoderNApi.prototype, 'encoding');
...
try {
  const b = {};
  TextEncoderNApi.prototype.encode.call(b, '');
} catch (e) {
  print(e.message);
  // CHECK-NEXT: TextEncoderNApi.prototype.encode() called on non-TextEncoderNApi object
}

try {
  TextEncoderNApi.prototype.encode.call(undefined, '');
} catch (e) {
  print(e.message);
  // CHECK-NEXT: TextEncoderNApi.prototype.encode() called on non-TextEncoderNApi object
}
...
try {
  const b = {};
  TextEncoderNApi.prototype.encodeInto.call(b, '', result);
} catch (e) {
  print(e.message);
  // CHECK-NEXT: TextEncoderNApi.prototype.encodeInto() called on non-TextEncoderNApi object
}

try {
  TextEncoderNApi.prototype.encodeInto.call(undefined, '', result);
} catch (e) {
  print(e.message);
  // CHECK-NEXT: TextEncoderNApi.prototype.encodeInto() called on non-TextEncoderNApi object
}
...
try {
  ...
  // CHECK-NEXT: TypeError TextEncoderNApi.prototype.encodeInto called on a detached ArrayBuffer
}
