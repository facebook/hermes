...
TEST_F(NapiModuleLoadTest, LoadNonExistent) {
  ...
  napi_value result;
  napi_status status = hermes_napi_load_module(
      env_, "/nonexistent/path/addon.node", &result);
...
TEST_F(NapiModuleLoadTest, LoadInvalidFile) {
  ...
  napi_value result;
  napi_status status =
      hermes_napi_load_module(env_, "/dev/null", &result);
  ASSERT_NE(status, napi_ok);
...
TEST_F(NapiModuleLoadTest, LoadTestAddon) {
  ...
  // Verify the "hello" property is set to "world".
  {
    ...
    // Look up "hello".
    auto symRes =
        rt_->getIdentifierTable().getSymbolHandle(
            *rt_, vm::ASCIIRef("hello", 5));
    ASSERT_EQ(symRes.getStatus(), vm::ExecutionStatus::RETURNED);

    auto propRes =
        vm::JSObject::getNamed_RJS(objHandle, *rt_, **symRes);
    ...
    EXPECT_EQ(
        llvh::StringRef(ref.data(), ref.size()),
        llvh::StringRef("world"));
  }
