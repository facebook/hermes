...
class NapiDefineClassTest : public NapiTestFixture {
 protected:
  ...
  bool sameValue(napi_value a, napi_value b) {
    auto *phvA =
        reinterpret_cast<vm::PinnedHermesValue *>(a);
    auto *phvB =
        reinterpret_cast<vm::PinnedHermesValue *>(b);
...
static napi_value TestConstructor(
    napi_env env,
    napi_callback_info info) {
  napi_value thisArg;
  void *data;
  EXPECT_EQ(napi_get_cb_info(env, info, nullptr, nullptr,
                              &thisArg, &data), napi_ok);
...
TEST_F(NapiDefineClassTest, BasicClassDefinition) {
  ...
  ASSERT_EQ(napi_define_class(
      env_, "MyClass", NAPI_AUTO_LENGTH,
      TestConstructor, cbData,
      0, nullptr, &cls), napi_ok);
...
TEST_F(NapiDefineClassTest, NullArgValidation) {
  ...
  // Null result.
  EXPECT_EQ(napi_define_class(
      env_, "MyClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      0, nullptr, nullptr), napi_invalid_arg);

  // Null constructor.
  EXPECT_EQ(napi_define_class(
      env_, "MyClass", NAPI_AUTO_LENGTH,
      nullptr, nullptr,
      0, nullptr, &cls), napi_invalid_arg);

  // Null properties when count > 0.
  EXPECT_EQ(napi_define_class(
      env_, "MyClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      1, nullptr, &cls), napi_invalid_arg);
}
...
TEST_F(NapiDefineClassTest, ConstructorCalledViaNewInstance) {
  ...
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "TestClass", NAPI_AUTO_LENGTH,
      TestConstructor, cbData,
      0, nullptr, &cls), napi_ok);

  // Create an instance.
  napi_value instance;
  ASSERT_EQ(napi_new_instance(
      env_, cls, 0, nullptr, &instance), napi_ok);
...
TEST_F(NapiDefineClassTest, PrototypeChain) {
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "ProtoClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      0, nullptr, &cls), napi_ok);
  ...
  ASSERT_EQ(napi_get_named_property(
      env_, cls, "prototype", &proto), napi_ok);
  ...
  napi_value ctorProp;
  ASSERT_EQ(napi_get_named_property(
      env_, proto, "constructor", &ctorProp), napi_ok);
  ...
  ASSERT_EQ(napi_get_prototype(
      env_, instance, &instanceProto), napi_ok);
...
TEST_F(NapiDefineClassTest, InstanceMethod) {
  napi_property_descriptor props[] = {
      {"getValue", nullptr, GetValue42, nullptr, nullptr, nullptr,
       napi_default, nullptr},
  };

  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "MethodClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      1, props, &cls), napi_ok);
  ...
  napi_value method;
  ASSERT_EQ(napi_get_named_property(
      env_, instance, "getValue", &method), napi_ok);

  napi_value result;
  ASSERT_EQ(napi_call_function(
      env_, instance, method, 0, nullptr, &result), napi_ok);
...
static napi_value StaticHello(
    napi_env env,
    napi_callback_info info) {
  napi_value result;
  EXPECT_EQ(napi_create_string_utf8(
      env, "hello", NAPI_AUTO_LENGTH, &result), napi_ok);
  return result;
}

TEST_F(NapiDefineClassTest, StaticMethodNotOnInstance) {
  napi_property_descriptor props[] = {
      {"staticFn", nullptr, StaticHello, nullptr, nullptr,
       nullptr,
  ...
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "StaticOnly", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      1, props, &cls), napi_ok);

  // Static method should be on constructor.
  bool hasOnCtor;
  ASSERT_EQ(napi_has_named_property(
      env_, cls, "staticFn", &hasOnCtor), napi_ok);
  ...
  ASSERT_EQ(napi_create_string_utf8(
      env_, "staticFn", NAPI_AUTO_LENGTH, &key), napi_ok);

  bool hasOnInstance;
  ASSERT_EQ(napi_has_own_property(
      env_, instance, key, &hasOnInstance), napi_ok);
...
TEST_F(NapiDefineClassTest, StaticMethodCallable) {
  ...
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "StaticCallable", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      1, props, &cls), napi_ok);
  ...
  napi_value method;
  ASSERT_EQ(napi_get_named_property(
      env_, cls, "staticFn", &method), napi_ok);
  ...
  napi_value result;
  ASSERT_EQ(napi_call_function(
      env_, global, method, 0, nullptr, &result), napi_ok);

  // Result should be "hello".
  size_t len;
  ASSERT_EQ(napi_get_value_string_utf8(
      env_, result, nullptr, 0, &len), napi_ok);
...
static napi_value GetAccessorValue(
    napi_env env,
    napi_callback_info info) {
  napi_value result;
  EXPECT_EQ(napi_create_int32(env, g_accessorValue, &result),
            napi_ok);
  return result;
}

static napi_value SetAccessorValue(
    napi_env env,
    napi_callback_info info) {
  size_t argc = 1;
  napi_value argv[1];
  EXPECT_EQ(napi_get_cb_info(
      env, info, &argc, argv, nullptr, nullptr), napi_ok);
...
TEST_F(NapiDefineClassTest, InstanceAccessor) {
  ...
  napi_property_descriptor props[] = {
      {"value", nullptr, nullptr, GetAccessorValue,
       SetAccessorValue, nullptr, napi_default, nullptr},
  };

  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "AccessorClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      1, props, &cls), napi_ok);
  ...
  ASSERT_EQ(napi_set_named_property(
      env_, instance, "value", setVal), napi_ok);
  EXPECT_EQ(g_accessorValue, 100);

  // Get via accessor.
  napi_value getVal;
  ASSERT_EQ(napi_get_named_property(
      env_, instance, "value", &getVal), napi_ok);
...
TEST_F(NapiDefineClassTest, InstanceDataProperty) {
  ...
  napi_property_descriptor props[] = {
      {"myData", nullptr, nullptr, nullptr, nullptr, dataVal,
       static_cast<napi_property_attributes>(
  ...
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "DataClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      1, props, &cls), napi_ok);
  ...
  napi_value result;
  ASSERT_EQ(napi_get_named_property(
      env_, instance, "myData", &result), napi_ok);
...
static napi_value ConstructorWithArgs(
    napi_env env,
    napi_callback_info info) {
  size_t argc = 1;
  napi_value argv[1];
  napi_value thisArg;
  EXPECT_EQ(napi_get_cb_info(
      env, info, &argc, argv, &thisArg, nullptr), napi_ok);

  if (argc >= 1) {
    // Store argument as .ctorArg on the instance.
    EXPECT_EQ(napi_set_named_property(
        env, thisArg, "ctorArg", argv[0]), napi_ok);
  }
...
TEST_F(NapiDefineClassTest, ConstructorReceivesArguments) {
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "ArgClass", NAPI_AUTO_LENGTH,
      ConstructorWithArgs, nullptr,
      0, nullptr, &cls), napi_ok);
  ...
  napi_value instance;
  ASSERT_EQ(napi_new_instance(
      env_, cls, 1, &arg, &instance), napi_ok);

  // Verify the constructor stored the argument.
  napi_value result;
  ASSERT_EQ(napi_get_named_property(
      env_, instance, "ctorArg", &result), napi_ok);
...
TEST_F(NapiDefineClassTest, MixedProperties) {
  ...
  napi_property_descriptor props[] = {
      // Instance method
      {"instanceMethod", nullptr, GetValue42, nullptr, nullptr,
       nullptr, napi_default, nullptr},
      // Static method
      {"staticMethod", nullptr, StaticHello, nullptr, nullptr,
       nullptr,
       static_cast<napi_property_attributes>(
           napi_default | napi_static),
       nullptr},
      // Static data property
      {"STATIC_VAL", nullptr, nullptr, nullptr, nullptr,
       staticVal,
       static_cast<napi_property_attributes>(
           napi_default | napi_static),
       nullptr},
  ...
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "MixedClass", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      3, props, &cls), napi_ok);
  ...
  bool hasProp;
  ASSERT_EQ(napi_has_named_property(
      env_, proto, "instanceMethod", &hasProp), napi_ok);
  EXPECT_TRUE(hasProp);

  // Static method on constructor.
  ASSERT_EQ(napi_has_named_property(
      env_, cls, "staticMethod", &hasProp), napi_ok);
  EXPECT_TRUE(hasProp);

  // Static data on constructor.
  ASSERT_EQ(napi_has_named_property(
      env_, cls, "STATIC_VAL", &hasProp), napi_ok);
  EXPECT_TRUE(hasProp);

  napi_value staticResult;
  ASSERT_EQ(napi_get_named_property(
      env_, cls, "STATIC_VAL", &staticResult), napi_ok);

  int32_t staticResultVal;
  ASSERT_EQ(napi_get_value_int32(
      env_, staticResult, &staticResultVal), napi_ok);
...
static napi_value WrappingConstructor(
    napi_env env,
    napi_callback_info info) {
  napi_value thisArg;
  EXPECT_EQ(napi_get_cb_info(
      env, info, nullptr, nullptr, &thisArg, nullptr), napi_ok);

  auto *td = new TestData{42};
  EXPECT_EQ(napi_wrap(
      env, thisArg, td, TestDataFinalize, nullptr, nullptr),
      napi_ok);

  return thisArg;
}

static napi_value GetWrappedValue(
    napi_env env,
    napi_callback_info info) {
  napi_value thisArg;
  EXPECT_EQ(napi_get_cb_info(
...
TEST_F(NapiDefineClassTest, WrapInsideConstructor) {
  napi_property_descriptor props[] = {
      {"getWrappedValue", nullptr, GetWrappedValue, nullptr,
       nullptr, nullptr, napi_default, nullptr},
  };

  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "WrappedClass", NAPI_AUTO_LENGTH,
      WrappingConstructor, nullptr,
      1, props, &cls), napi_ok);
  ...
  napi_value method;
  ASSERT_EQ(napi_get_named_property(
      env_, instance, "getWrappedValue", &method), napi_ok);
...
TEST_F(NapiDefineClassTest, MultipleInstancesSharePrototype) {
  napi_value cls;
  ASSERT_EQ(napi_define_class(
      env_, "SharedProto", NAPI_AUTO_LENGTH,
      TestConstructor, nullptr,
      0, nullptr, &cls), napi_ok);

  napi_value inst1, inst2;
  ASSERT_EQ(napi_new_instance(
      env_, cls, 0, nullptr, &inst1), napi_ok);
  ASSERT_EQ(napi_new_instance(
      env_, cls, 0, nullptr, &inst2), napi_ok);
