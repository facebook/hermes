...
TEST_F(NapiCallFunctionTest, CallWithArgs) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_call_function(env_, recv, func, 2, args, &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiCallFunctionTest, CallReceivesThisArg) {
  ...
  napi_callback cb = [](napi_env env,
    ...
    napi_get_cb_info(
        env, info, nullptr, nullptr, &this_arg, nullptr);
    return this_arg;
  };
  ...
  EXPECT_EQ(
      napi_ok,
      napi_set_named_property(env_, thisObj, "marker", marker));

  napi_value result = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_call_function(
          env_, thisObj, func, 0, nullptr, &result));
  ASSERT_NE(nullptr, result);
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(env_, result, "marker", &got));
...
TEST_F(NapiCallFunctionTest, ResultIsOptional) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_call_function(
          env_, recv, func, 0, nullptr, nullptr));
...
TEST_F(NapiCallFunctionTest, CalleeThrowsSetsException) {
  ...
  EXPECT_EQ(
      napi_pending_exception,
      napi_call_function(
          env_, recv, func, 0, nullptr, &result));
...
TEST_F(NapiCallFunctionTest, CallWithManyArgs) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_create_function(
          env_, "sum", NAPI_AUTO_LENGTH, cb, nullptr, &func));
  ...
  EXPECT_EQ(
      napi_ok,
      napi_call_function(env_, recv, func, 10, args, &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiCallFunctionTest, ArgcNonZeroArgvNull) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_call_function(
          env_, recv, func, 1, nullptr, nullptr));
...
TEST_F(NapiCallFunctionTest, CallFunctionOnObject) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_set_named_property(env_, obj, "x", xVal));
  ...
  EXPECT_EQ(
      napi_ok,
      napi_set_named_property(env_, obj, "getValue", method));

  // Get the method and call it with obj as 'this'.
  napi_value gotMethod = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(env_, obj, "getValue", &gotMethod));

  napi_value result = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_call_function(
          env_, obj, gotMethod, 0, nullptr, &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiCallFunctionTest, CallStringFunc) {
  ...
  EXPECT_EQ(
      ...
      napi_get_value_string_utf8(
          env_, result, buf, sizeof(buf), &len));
...
TEST_F(NapiCallFunctionTest, CallWithZeroArgcAndNonNullArgv) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_call_function(
          env_, recv, func, 0, dummyArgv, &result));
  ASSERT_NE(nullptr, result);
