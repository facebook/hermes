...
TEST_F(NapiComparisonTest, StrictEqualsNullArgs) {
  ...
  // Null env.
  EXPECT_EQ(
      napi_strict_equals(nullptr, val, val, &result),
      napi_invalid_arg);
  // Null lhs.
  EXPECT_EQ(
      napi_strict_equals(env_, nullptr, val, &result),
      napi_invalid_arg);
  // Null rhs.
  EXPECT_EQ(
      napi_strict_equals(env_, val, nullptr, &result),
      napi_invalid_arg);
  // Null result.
  EXPECT_EQ(
      napi_strict_equals(env_, val, val, nullptr),
      napi_invalid_arg);
  closeScope();
}
...
TEST_F(NapiComparisonTest, StrictEqualsSameString) {
  ...
  napi_value a, b;
  ASSERT_EQ(
      napi_create_string_utf8(env_, "hello", NAPI_AUTO_LENGTH, &a),
      napi_ok);
  ASSERT_EQ(
      napi_create_string_utf8(env_, "hello", NAPI_AUTO_LENGTH, &b),
      napi_ok);
...
TEST_F(NapiComparisonTest, StrictEqualsDifferentStrings) {
  ...
  ASSERT_EQ(
      ...
      napi_ok);
  ASSERT_EQ(
      napi_create_string_utf8(env_, "world", NAPI_AUTO_LENGTH, &b),
      napi_ok);
...
TEST_F(NapiComparisonTest, StrictEqualsNumberVsString) {
  ...
  ASSERT_EQ(
      napi_create_string_utf8(env_, "42", NAPI_AUTO_LENGTH, &str),
      napi_ok);
...
TEST_F(NapiComparisonTest, InstanceofNullArgs) {
  ...
  // Null env.
  EXPECT_EQ(
      napi_instanceof(nullptr, obj, ctor, &result),
      napi_invalid_arg);
  // Null object.
  EXPECT_EQ(
      napi_instanceof(env_, nullptr, ctor, &result),
      napi_invalid_arg);
  // Null result.
  EXPECT_EQ(
      napi_instanceof(env_, obj, ctor, nullptr),
      napi_invalid_arg);
  closeScope();
}
...
TEST_F(NapiComparisonTest, InstanceofNonFunction) {
  ...
  EXPECT_EQ(
      napi_instanceof(env_, obj, notFunc, &result),
...
TEST_F(NapiComparisonTest, InstanceofWithDefineClass) {
  ...
  // Create an instance.
  napi_value instance;
  ASSERT_EQ(napi_new_instance(env_, ctor, 0, nullptr, &instance),
            napi_ok);

  // instanceof should return true.
  bool result = false;
  ASSERT_EQ(
      napi_instanceof(env_, instance, ctor, &result), napi_ok);
  EXPECT_TRUE(result);
  ...
  result = true;
  ASSERT_EQ(
      napi_instanceof(env_, plain, ctor, &result), napi_ok);
  EXPECT_FALSE(result);
  closeScope();
}
...
TEST_F(NapiComparisonTest, InstanceofPrimitiveObject) {
  ...
  ASSERT_EQ(
      napi_get_named_property(env_, global, "Array", &arrayCtor),
      napi_ok);
  ...
  bool result = false;
  ASSERT_EQ(
      napi_instanceof(env_, arr, arrayCtor, &result), napi_ok);
  EXPECT_TRUE(result);
  ...
  result = true;
  ASSERT_EQ(
      napi_instanceof(env_, num, arrayCtor, &result), napi_ok);
  EXPECT_FALSE(result);
  closeScope();
}
...
TEST_F(NapiComparisonTest, InstanceofWithPendingException) {
  ...
  ASSERT_EQ(
      napi_create_string_utf8(
          env_, "test error", NAPI_AUTO_LENGTH, &errVal),
      napi_ok);
  ...
  bool result;
  EXPECT_EQ(
      napi_instanceof(env_, obj, ctor, &result),
