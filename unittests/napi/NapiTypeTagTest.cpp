...
TEST_F(NapiTestFixture, TypeTagObject_NullEnv) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_type_tag_object(nullptr, nullptr, &tag));
}
...
TEST_F(NapiTestFixture, TypeTagObject_NullValue) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_type_tag_object(env_, nullptr, &tag));
...
TEST_F(NapiTestFixture, TypeTagObject_NullTag) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_type_tag_object(env_, obj, nullptr));
...
TEST_F(NapiTestFixture, TypeTagObject_BasicObject) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj, &tag, &result));
  EXPECT_TRUE(result);
...
TEST_F(NapiTestFixture, TypeTagObject_RetagFails) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_type_tag_object(env_, obj, &tag2));

  // Original tag should still be valid.
  bool result = false;
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj, &tag1, &result));
  EXPECT_TRUE(result);
...
TEST_F(NapiTestFixture, TypeTagObject_WrongTagReturnsFalse) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj, &wrongTag, &result));
  EXPECT_FALSE(result);
...
TEST_F(NapiTestFixture, TypeTagObject_ZeroTag) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj, &zeroTag, &result));
  EXPECT_TRUE(result);
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj, &otherTag, &result));
  EXPECT_FALSE(result);
...
TEST_F(NapiTestFixture, TypeTagExternal_Basic) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_external(env_, &nativeData, nullptr, nullptr, &ext));
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, ext, &tag, &result));
  EXPECT_TRUE(result);
...
TEST_F(NapiTestFixture, TypeTagExternal_RetagFails) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_external(env_, nullptr, nullptr, nullptr, &ext));
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_type_tag_object(env_, ext, &tag2));

  // Original tag should still be valid.
  bool result = false;
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, ext, &tag1, &result));
  EXPECT_TRUE(result);
...
TEST_F(NapiTestFixture, TypeTagExternal_WrongTagReturnsFalse) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, ext, &wrongTag, &result));
  EXPECT_FALSE(result);
...
TEST_F(NapiTestFixture, CheckTypeTag_NullResult) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_check_object_type_tag(env_, obj, &tag, nullptr));
...
TEST_F(NapiTestFixture, CheckTypeTag_NonObjectReturnsFalse) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, num, &tag, &result));
  EXPECT_FALSE(result);
...
TEST_F(NapiTestFixture, TypeTag_MultipleObjects) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj1, &tag1, &result));
  EXPECT_TRUE(result);

  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj2, &tag2, &result));
  EXPECT_TRUE(result);

  // And not match the other's tag.
  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj1, &tag2, &result));
  EXPECT_FALSE(result);

  ASSERT_EQ(
      napi_ok,
      napi_check_object_type_tag(env_, obj2, &tag1, &result));
  EXPECT_FALSE(result);
