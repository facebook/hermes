...
TEST_F(NapiRunScriptTest, ObjectExpression) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_get_named_property(env_, result, "a", &aProp));
  double aVal;
  ...
  ASSERT_EQ(
      napi_ok,
      napi_get_named_property(env_, result, "b", &bProp));
  char buf[32];
  size_t len;
  ASSERT_EQ(
...
TEST_F(NapiRunScriptTest, SyntaxError) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(
          env_, "function(", NAPI_AUTO_LENGTH, &script));
...
TEST_F(NapiRunScriptTest, NullArgs) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "1", NAPI_AUTO_LENGTH, &script));
