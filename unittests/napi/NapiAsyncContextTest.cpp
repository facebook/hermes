...
TEST_F(NapiAsyncContextTest, NullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_async_init(nullptr, nullptr, nullptr, nullptr));
}
...
TEST_F(NapiAsyncContextTest, NullResourceName) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_async_init(env_, nullptr, nullptr, &ctx));
...
TEST_F(NapiAsyncContextTest, NullResult) {
  ...
  EXPECT_EQ(
      ...
      napi_create_string_utf8(
          env_, "test", NAPI_AUTO_LENGTH, &name));

  EXPECT_EQ(
      napi_invalid_arg,
      napi_async_init(env_, nullptr, name, nullptr));
...
TEST_F(NapiAsyncContextTest, CreateAndDestroy) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_create_string_utf8(
          env_, "test_resource", NAPI_AUTO_LENGTH, &name));
...
TEST_F(NapiAsyncContextTest, CreateWithResource) {
  ...
  EXPECT_EQ(
      napi_ok, napi_create_object(env_, &resource));
...
TEST_F(NapiAsyncContextTest, DestroyNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_async_destroy(nullptr, nullptr));
}

TEST_F(NapiAsyncContextTest, DestroyNullContext) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_async_destroy(env_, nullptr));
}
...
TEST_F(NapiMakeCallbackTest, NullRecv) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_make_callback(
          env_, nullptr, nullptr, func, 0, nullptr, nullptr));
...
TEST_F(NapiMakeCallbackTest, NullFunc) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_make_callback(
          env_, nullptr, recv, nullptr, 0, nullptr, nullptr));
...
TEST_F(NapiMakeCallbackTest, SimpleCall) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_make_callback(
          env_, nullptr, recv, func, 0, nullptr, &result));
...
TEST_F(NapiMakeCallbackTest, CallWithAsyncContext) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_make_callback(
          env_, ctx, recv, func, 1, &arg, &result));
...
TEST_F(NapiMakeCallbackTest, CallWithNullResult) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_make_callback(
          env_, nullptr, recv, func, 0, nullptr, nullptr));
...
TEST_F(NapiMakeCallbackTest, NonCallableFunc) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_make_callback(
          env_, nullptr, recv, num, 0, nullptr, nullptr));
...
TEST_F(NapiMakeCallbackTest, ThisArgPassed) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_set_named_property(env_, obj, "x", val));

  napi_value result = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_make_callback(
          env_, nullptr, obj, func, 0, nullptr, &result));

  int32_t extracted = 0;
  EXPECT_EQ(
      napi_ok, napi_get_value_int32(env_, result, &extracted));
