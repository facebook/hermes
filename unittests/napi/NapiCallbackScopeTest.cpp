...
TEST_F(NapiCallbackScopeTest, OpenNullResult) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_open_callback_scope(
          env_, nullptr, nullptr, nullptr));
}

TEST_F(NapiCallbackScopeTest, CloseNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_close_callback_scope(nullptr, nullptr));
}

TEST_F(NapiCallbackScopeTest, CloseNullScope) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_close_callback_scope(env_, nullptr));
}
...
TEST_F(NapiCallbackScopeTest, NestedScopes) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_open_callback_scope(
          env_, nullptr, nullptr, &cbScope1));

  napi_callback_scope cbScope2 = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_open_callback_scope(
          env_, nullptr, nullptr, &cbScope2));
...
TEST_F(NapiCallbackScopeTest, CloseMismatch) {
  ...
  napi_callback_scope fakeScope =
      reinterpret_cast<napi_callback_scope>(1);
  EXPECT_EQ(
      napi_callback_scope_mismatch,
      napi_close_callback_scope(env_, fakeScope));
}
