...
TEST_F(NapiCleanupHookTest, AddNull) {
  ...
  EXPECT_EQ(
      napi_add_env_cleanup_hook(scope.env, nullptr, nullptr),
      napi_invalid_arg);
}
...
TEST_F(NapiCleanupHookTest, HookRemovesItself) {
  ...
  SelfRemoveData data2;
  {
    ...
    ASSERT_EQ(
        napi_add_env_cleanup_hook(
            scope.env, selfRemovingHook, &data1),
        napi_ok);
    ASSERT_EQ(
        napi_add_env_cleanup_hook(
            scope.env, selfRemovingHook, &data2),
        napi_ok);
  }
...
TEST_F(NapiCleanupHookTest, AsyncAddNull) {
  ...
  EXPECT_EQ(
      napi_add_async_cleanup_hook(
          nullptr, asyncRecordHook, nullptr, &handle),
      napi_invalid_arg);
  // Null hook should return napi_invalid_arg.
  EXPECT_EQ(
      napi_add_async_cleanup_hook(
          scope.env, nullptr, nullptr, &handle),
      napi_invalid_arg);
}
