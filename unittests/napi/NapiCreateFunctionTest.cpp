  ...
  switch (args.size()) {
    ...
    case 1:
      return Callable::executeCall1(
          funcHandle, runtime, thisArg, args[0]);
    case 2:
...
TEST_F(NapiCreateFunctionTest, NullEnv) {
  ...
  EXPECT_EQ(napi_invalid_arg, napi_create_function(
      nullptr, nullptr, 0, cb, nullptr, &result));
}
...
TEST_F(NapiCreateFunctionTest, NullResult) {
  ...
  EXPECT_EQ(napi_invalid_arg, napi_create_function(
      env_, nullptr, 0, cb, nullptr, nullptr));
}
...
TEST_F(NapiCreateFunctionTest, NullCallback) {
  ...
  napi_value result = nullptr;
  EXPECT_EQ(napi_invalid_arg, napi_create_function(
      env_, nullptr, 0, nullptr, nullptr, &result));
...
TEST_F(NapiCreateFunctionTest, CreateAnonymous) {
  ...
  EXPECT_EQ(napi_ok, napi_create_function(
      env_, nullptr, 0, cb, nullptr, &func));
...
TEST_F(NapiCreateFunctionTest, CreateNamed) {
  ...
  EXPECT_EQ(napi_ok, napi_create_string_utf8(
      env_, "name", NAPI_AUTO_LENGTH, &nameKey));
  ...
  char buf[32] = {};
  size_t len = 0;
  EXPECT_EQ(napi_ok, napi_get_value_string_utf8(
      env_, nameVal, buf, sizeof(buf), &len));
...
TEST_F(NapiCreateFunctionTest, CallWithArgsAddsNumbers) {
  ...
  napi_value func = nullptr;
  EXPECT_EQ(napi_ok, napi_create_function(
      env_, "add", NAPI_AUTO_LENGTH, cb, nullptr, &func));
...
TEST_F(NapiCreateFunctionTest, CallbackReceivesThisArg) {
  ...
  auto res = callFunction(
      env_->runtime,
      func,
      env_->runtime.makeHandle(
          HermesValue::encodeTrustedNumberValue(99)),
      {});
  ASSERT_NE(ExecutionStatus::EXCEPTION, res.getStatus());
  EXPECT_TRUE(res->get().isNumber());
...
TEST_F(NapiCreateFunctionTest, CallbackReceivesData) {
  ...
  napi_value func = nullptr;
  EXPECT_EQ(napi_ok, napi_create_function(
      env_, nullptr, 0, cb, &userData, &func));
...
TEST_F(NapiCreateFunctionTest, PendingExceptionBlocksCreate) {
  ...
  napi_value func = nullptr;
  EXPECT_EQ(napi_pending_exception, napi_create_function(
      env_, nullptr, 0, cb, nullptr, &func));
...
TEST_F(NapiCreateFunctionTest, MultipleCreatedFunctions) {
  ...
  napi_callback cb1 = [](napi_env env,
                          napi_callback_info) -> napi_value {
    napi_value result = nullptr;
    napi_create_double(env, 1.0, &result);
    return result;
  };
  napi_callback cb2 = [](napi_env env,
                          napi_callback_info) -> napi_value {
    napi_value result = nullptr;
  ...
  {
    GCScope gcScope(env_->runtime);
    auto res1 = callFunction(
        env_->runtime, func1, Runtime::getUndefinedValue(), {});
  ...
  {
    GCScope gcScope(env_->runtime);
    auto res2 = callFunction(
        env_->runtime, func2, Runtime::getUndefinedValue(), {});
