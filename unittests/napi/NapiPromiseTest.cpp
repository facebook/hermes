...
TEST_F(NapiPromiseTest, CreatePromise_NullDeferred) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_create_promise(env_, nullptr, &promise));
...
TEST_F(NapiPromiseTest, CreatePromise_NullPromise) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_create_promise(env_, &deferred, nullptr));
...
TEST_F(NapiPromiseTest, ResolveDeferred_Basic) {
  ...
  ASSERT_EQ(
      napi_ok, napi_create_double(env_, 42.0, &resolution));
  ASSERT_EQ(
      napi_ok, napi_resolve_deferred(env_, deferred, resolution));
...
TEST_F(NapiPromiseTest, RejectDeferred_Basic) {
  ...
  ASSERT_EQ(
      ...
      napi_create_string_utf8(
          env_, "error", NAPI_AUTO_LENGTH, &rejection));
  ASSERT_EQ(
      napi_ok, napi_reject_deferred(env_, deferred, rejection));
...
TEST_F(NapiPromiseTest, IsPromise_NullValue) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_is_promise(env_, nullptr, &is_promise));
}
...
TEST_F(NapiPromiseTest, IsPromise_NullResult) {
  ...
  ASSERT_EQ(napi_ok, napi_create_object(env_, &obj));
  EXPECT_EQ(
      napi_invalid_arg, napi_is_promise(env_, obj, nullptr));
...
static napi_value
captureCallback(napi_env env, napi_callback_info info) {
  size_t argc = 1;
  napi_value argv[1];
...
TEST_F(NapiPromiseTest, ResolveDeferred_ThenCallback) {
  ...
  ASSERT_EQ(
      ...
      napi_create_function(
          env_, "capture", NAPI_AUTO_LENGTH, captureCallback,
          nullptr, &cb));
  ...
  ASSERT_EQ(
      napi_ok,
      napi_get_named_property(env_, promise, "then", &then_result));
  napi_value then_promise = nullptr;
  ASSERT_EQ(
      napi_ok,
      napi_call_function(
          env_, promise, then_result, 1, &cb, &then_promise));
...
TEST_F(NapiPromiseTest, RejectDeferred_CatchCallback) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_get_named_property(env_, promise, "catch", &catch_fn));
  napi_value catch_promise = nullptr;
  ASSERT_EQ(
      napi_ok,
      napi_call_function(
          env_, promise, catch_fn, 1, &cb, &catch_promise));
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(
          env_, "oops", NAPI_AUTO_LENGTH, &rejection));
  ASSERT_EQ(
      napi_ok, napi_reject_deferred(env_, deferred, rejection));
  ...
  ASSERT_EQ(
      ...
      napi_get_value_string_utf8(
          env_, capturedValue, buf, sizeof(buf), &len));
