...
TEST_F(NapiAsyncWorkTest, CancelAsyncWork_AlreadyExecuted) {
  ...
  EXPECT_EQ(
      napi_cancel_async_work(env_, work), napi_generic_failure);
...
TEST_F(NapiAsyncWorkTest, MultipleWorkItems) {
  ...
  for (int i = 0; i < 3; ++i) {
    ASSERT_EQ(
        napi_create_async_work(
            env_, nullptr, nullptr, execute, complete, &counts[i],
            &works[i]),
        napi_ok);
...
TEST_F(NapiAsyncWorkTest, FatalExceptionCallsHostHandler) {
  ...
  eventLoop_.fatal_exception =
      [](void *data, napi_env env, napi_value err) {
