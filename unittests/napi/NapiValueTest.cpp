...
TEST_F(NapiTestFixture, CreateInt32_MaxValue) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_int32(
          env_, std::numeric_limits<int32_t>::max(), &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiTestFixture, CreateInt32_MinValue) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_int32(
          env_, std::numeric_limits<int32_t>::min(), &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiTestFixture, CreateUint32_MaxValue) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_uint32(
          env_, std::numeric_limits<uint32_t>::max(), &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiTestFixture, CreateInt64_MaxValue) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_int64(
          env_, std::numeric_limits<int64_t>::max(), &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiTestFixture, CreateInt64_MinValue) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_int64(
          env_, std::numeric_limits<int64_t>::min(), &result));
  ASSERT_NE(nullptr, result);
...
TEST_F(NapiTestFixture, GetValueUint32_NullEnv) {
  EXPECT_EQ(
      napi_invalid_arg, napi_get_value_uint32(nullptr, nullptr, nullptr));
}
...
TEST_F(NapiTestFixture, CreateSymbol_TwoSymbolsNotEqual) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "same", NAPI_AUTO_LENGTH, &desc));
...
TEST_F(NapiTestFixture, SymbolFor_DifferentKeyDifferentSymbol) {
  ...
  ASSERT_EQ(
      napi_ok,
      node_api_symbol_for(env_, "key_a", NAPI_AUTO_LENGTH, &sym1));
  ASSERT_EQ(
      napi_ok,
      node_api_symbol_for(env_, "key_b", NAPI_AUTO_LENGTH, &sym2));
...
TEST_F(NapiTestFixture, SymbolFor_TypeofIsSymbol) {
  ...
  ASSERT_EQ(
      napi_ok,
      node_api_symbol_for(env_, "test", NAPI_AUTO_LENGTH, &sym));
...
TEST_F(NapiTestFixture, CreateSymbol_VsSymbolFor_NotEqual) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "test", NAPI_AUTO_LENGTH, &desc));
  ...
  ASSERT_EQ(
      napi_ok,
      node_api_symbol_for(env_, "test", NAPI_AUTO_LENGTH, &sym2));
...
TEST_F(NapiTestFixture, GetDateValue_NullEnv) {
  EXPECT_EQ(
      napi_invalid_arg, napi_get_date_value(nullptr, nullptr, nullptr));
}

TEST_F(NapiTestFixture, GetDateValue_NullValue) {
  double result = 0;
  EXPECT_EQ(
      napi_invalid_arg, napi_get_date_value(env_, nullptr, &result));
}
