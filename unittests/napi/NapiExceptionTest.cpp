...
TEST_F(NapiTestFixture, PreambleRejectsWhenExceptionPending) {
  ...
  env_->pendingException =
      hermes::vm::HermesValue::encodeUndefinedValue();

  int out = 0;
  EXPECT_EQ(
      napi_pending_exception, testPreambleFunction(env_, &out));
...
TEST_F(NapiTestFixture, ExceptionSafeEscapableHandleScopes) {
  ...
  EXPECT_EQ(
      napi_ok, napi_open_escapable_handle_scope(env_, &escScope));
  EXPECT_EQ(
      napi_ok, napi_close_escapable_handle_scope(env_, escScope));
...
TEST_F(NapiTestFixture, ExceptionSafeStringExtraction) {
  ...
  size_t len;
  EXPECT_EQ(
      napi_ok, napi_get_value_string_utf8(env_, val, nullptr, 0, &len));
  EXPECT_EQ(5u, len);
...
TEST_F(NapiTestFixture, PendingExceptionIsGCRooted) {
  ...
  napi_value str;
  ASSERT_EQ(
      napi_ok, napi_create_string_utf8(env_, "test error", 10, &str));
  ...
  auto *strPrim =
      hermes::vm::vmcast<hermes::vm::StringPrimitive>(
          env_->pendingException);
...
TEST_F(NapiTestFixture, ThrowStringValue) {
  ...
  napi_value str;
  ASSERT_EQ(
      napi_ok, napi_create_string_utf8(env_, "custom error", 12, &str));
...
TEST_F(NapiTestFixture, ThrowErrorNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg, napi_throw_error(nullptr, nullptr, "msg"));
}
...
TEST_F(NapiTestFixture, ThrowErrorNullMsg) {
  ...
  EXPECT_EQ(
      napi_invalid_arg, napi_throw_error(env_, nullptr, nullptr));
...
TEST_F(NapiTestFixture, ThrowErrorRejectsWhenExceptionPending) {
  ...
  EXPECT_EQ(
      ...
      napi_throw_error(env_, nullptr, "test error"));
...
TEST_F(NapiTestFixture, ThrowErrorSetsErrorObject) {
  ...
  EXPECT_TRUE(
      hermes::vm::vmisa<JSError>(env_->pendingException));
...
TEST_F(NapiTestFixture, ThrowErrorWithCode) {
  ...
  EXPECT_EQ(
      napi_ok, napi_throw_error(env_, "ERR_TEST", "test error"));
  ...
  // Verify the "code" property is set by reading it through VM APIs.
  {
    ...
    auto codeSymRes =
        env_->runtime.getIdentifierTable().getSymbolHandle(
            env_->runtime,
            hermes::vm::ASCIIRef{"code", 4});
    ASSERT_NE(
        hermes::vm::ExecutionStatus::EXCEPTION, codeSymRes.getStatus());
    ...
    auto codeRes = JSObject::getNamed_RJS(
        ...
        **codeSymRes);
    ASSERT_NE(
        hermes::vm::ExecutionStatus::EXCEPTION, codeRes.getStatus());

    // Verify it's the string "ERR_TEST".
    auto codeVal = codeRes->get();
    ASSERT_TRUE(codeVal.isString());
    auto *codeStr =
        hermes::vm::vmcast<StringPrimitive>(codeVal);
...
TEST_F(NapiTestFixture, ThrowErrorMessageIsCorrect) {
  ...
  EXPECT_EQ(
      napi_ok, napi_throw_error(env_, nullptr, "hello world"));
  ...
  // Verify the "message" property.
  {
    ...
    ASSERT_NE(
        hermes::vm::ExecutionStatus::EXCEPTION, msgRes.getStatus());

    auto msgVal = msgRes->get();
    ASSERT_TRUE(msgVal.isString());
    auto *msgStr =
        hermes::vm::vmcast<StringPrimitive>(msgVal);
    EXPECT_TRUE(msgStr->isASCII());
    auto ref = msgStr->getStringRef<char>();
    EXPECT_EQ(
        std::string("hello world"),
        std::string(ref.data(), ref.size()));
  }
...
TEST_F(NapiTestFixture, ThrowTypeErrorNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_throw_type_error(nullptr, nullptr, "msg"));
}
...
TEST_F(NapiTestFixture, ThrowTypeErrorNullMsg) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_throw_type_error(env_, nullptr, nullptr));
...
TEST_F(NapiTestFixture, ThrowTypeErrorSetsTypeError) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_throw_type_error(env_, nullptr, "bad type"));
  ...
  // Verify it has the TypeError prototype chain by checking "name".
  {
    ...
    auto nameRes = JSObject::getNamed_RJS(
        ...
        env_->runtime,
        hermes::vm::Predefined::getSymbolID(
            hermes::vm::Predefined::name));
    ASSERT_NE(
        hermes::vm::ExecutionStatus::EXCEPTION, nameRes.getStatus());

    auto nameVal = nameRes->get();
    ASSERT_TRUE(nameVal.isString());
    auto *nameStr =
        hermes::vm::vmcast<StringPrimitive>(nameVal);
...
TEST_F(NapiTestFixture, ThrowTypeErrorWithCode) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_throw_type_error(env_, "ERR_TYPE", "type error"));
...
TEST_F(NapiTestFixture, ThrowRangeErrorNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_throw_range_error(nullptr, nullptr, "msg"));
}
...
TEST_F(NapiTestFixture, ThrowRangeErrorNullMsg) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_throw_range_error(env_, nullptr, nullptr));
...
TEST_F(NapiTestFixture, ThrowRangeErrorSetsRangeError) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_throw_range_error(env_, nullptr, "out of range"));
...
TEST_F(NapiTestFixture, ThrowRangeErrorWithCode) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_throw_range_error(env_, "ERR_RANGE", "range error"));
  ...
  // Verify both "message" and "code" properties.
  {
    ...
    EXPECT_EQ(
        ...
        std::string(msgRef.data(), msgRef.size()));
    ...
    EXPECT_EQ(
        std::string("ERR_RANGE"),
        std::string(codeRef.data(), codeRef.size()));
  }
...
TEST_F(NapiTestFixture, ThrowErrorWithNullCode) {
  ...
  EXPECT_EQ(
      napi_ok, napi_throw_error(env_, nullptr, "no code"));
...
TEST_F(NapiTestFixture, ThrowErrorExceptionSurvivesGC) {
  ...
  EXPECT_EQ(
      napi_ok, napi_throw_error(env_, "GC_TEST", "gc test error"));
...
TEST_F(NapiTestFixture, GetAndClearLastExceptionNullEnv) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_get_and_clear_last_exception(nullptr, &result));
}

TEST_F(NapiTestFixture, GetAndClearLastExceptionNullResult) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_get_and_clear_last_exception(env_, nullptr));
}
...
TEST_F(NapiTestFixture, GetAndClearReturnsExceptionAndClears) {
  ...
  // Throw an error.
  ASSERT_EQ(
      napi_ok, napi_throw_error(env_, nullptr, "test exception"));
  ASSERT_TRUE(env_->hasPendingException);
...
TEST_F(NapiTestFixture, GetAndClearAllowsSubsequentAPICalls) {
  ...
  // Throw an error.
  ASSERT_EQ(
      napi_ok, napi_throw_error(env_, nullptr, "blocking error"));
  ASSERT_TRUE(env_->hasPendingException);

  // While exception is pending, NAPI_PREAMBLE functions should fail.
  EXPECT_EQ(
      napi_pending_exception,
      napi_throw_error(env_, nullptr, "second"));
  ...
  EXPECT_EQ(
      napi_ok, napi_throw_error(env_, nullptr, "now it works"));
  EXPECT_TRUE(env_->hasPendingException);
...
TEST_F(NapiTestFixture, GetAndClearExceptionSurvivesInScope) {
  ...
  napi_value str;
  ASSERT_EQ(
      napi_ok, napi_create_string_utf8(env_, "error msg", 9, &str));
...
TEST_F(NapiTestFixture, GetAndClearCalledTwice) {
  ...
  napi_value result1;
  EXPECT_EQ(
      napi_ok, napi_get_and_clear_last_exception(env_, &result1));
  EXPECT_FALSE(env_->hasPendingException);
  ...
  napi_value result2;
  EXPECT_EQ(
      napi_ok, napi_get_and_clear_last_exception(env_, &result2));
...
TEST_F(NapiTestFixture, CreateErrorNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_create_error(nullptr, nullptr, nullptr, nullptr));
}
...
TEST_F(NapiTestFixture, CreateErrorNullMsg) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_create_error(env_, nullptr, nullptr, &result));
...
TEST_F(NapiTestFixture, CreateErrorNullResult) {
  ...
  napi_value msg;
  ASSERT_EQ(
      napi_ok, napi_create_string_utf8(env_, "err", 3, &msg));
  EXPECT_EQ(
      napi_invalid_arg,
      napi_create_error(env_, nullptr, msg, nullptr));
...
TEST_F(NapiTestFixture, CreateErrorMsgMustBeString) {
  ...
  EXPECT_EQ(
      ...
      napi_create_error(env_, nullptr, num, &result));
...
TEST_F(NapiTestFixture, CreateErrorCodeMustBeString) {
  ...
  EXPECT_EQ(
      napi_string_expected,
      napi_create_error(env_, num, msg, &result));
...
TEST_F(NapiTestFixture, CreateErrorRejectsWhenExceptionPending) {
  ...
  EXPECT_EQ(
      napi_pending_exception,
      napi_create_error(env_, nullptr, msg, &result));
...
TEST_F(NapiTestFixture, CreateErrorBasic) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "test error", 10, &msg));
...
TEST_F(NapiTestFixture, CreateErrorWithCode) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "coded error", 11, &msg));
  napi_value code;
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "ERR_TEST", 8, &code));
  ...
  // Verify code property.
  {
    ...
    ASSERT_NE(
        ...
        codeSymRes.getStatus());
    EXPECT_EQ(
        "ERR_TEST",
        getStringProperty(env_, error, **codeSymRes));
  }
...
TEST_F(NapiTestFixture, CreateErrorWithNullCode) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "no code", 7, &msg));
  ...
  // Verify code property is undefined.
  {
    ...
    ASSERT_NE(
        ...
        codeSymRes.getStatus());
    auto *phv =
        reinterpret_cast<hermes::vm::PinnedHermesValue *>(error);
    ...
    auto res = hermes::vm::JSObject::getNamed_RJS(
        ...
        **codeSymRes);
    ASSERT_NE(
        hermes::vm::ExecutionStatus::EXCEPTION, res.getStatus());
...
TEST_F(NapiTestFixture, CreateTypeErrorBasic) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "type err", 8, &msg));

  napi_value error;
  EXPECT_EQ(
      napi_ok, napi_create_type_error(env_, nullptr, msg, &error));
  ASSERT_NE(nullptr, error);
...
TEST_F(NapiTestFixture, CreateTypeErrorWithCode) {
  ...
  napi_value msg;
  ASSERT_EQ(
      napi_ok, napi_create_string_utf8(env_, "msg", 3, &msg));
  napi_value code;
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "ERR_TYPE", 8, &code));

  napi_value error;
  EXPECT_EQ(
      napi_ok, napi_create_type_error(env_, code, msg, &error));
...
TEST_F(NapiTestFixture, CreateRangeErrorBasic) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "range err", 9, &msg));

  napi_value error;
  EXPECT_EQ(
      napi_ok,
      napi_create_range_error(env_, nullptr, msg, &error));
  ASSERT_NE(nullptr, error);
...
TEST_F(NapiTestFixture, CreateRangeErrorWithCode) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "ERR_RANGE", 9, &code));

  napi_value error;
  EXPECT_EQ(
      napi_ok, napi_create_range_error(env_, code, msg, &error));
...
TEST_F(NapiTestFixture, ThrowSyntaxErrorNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      node_api_throw_syntax_error(nullptr, nullptr, "msg"));
}
...
TEST_F(NapiTestFixture, ThrowSyntaxErrorNullMsg) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      node_api_throw_syntax_error(env_, nullptr, nullptr));
...
TEST_F(NapiTestFixture, ThrowSyntaxErrorSetsSyntaxError) {
  ...
  EXPECT_EQ(
      napi_ok,
      node_api_throw_syntax_error(env_, nullptr, "bad syntax"));
...
TEST_F(NapiTestFixture, CreateSyntaxErrorBasic) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "syntax err", 10, &msg));

  napi_value error;
  EXPECT_EQ(
      napi_ok,
      node_api_create_syntax_error(env_, nullptr, msg, &error));
  ASSERT_NE(nullptr, error);
...
TEST_F(NapiTestFixture, CreateSyntaxErrorWithCode) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "ERR_SYNTAX", 10, &code));

  napi_value error;
  EXPECT_EQ(
      napi_ok,
      node_api_create_syntax_error(env_, code, msg, &error));
...
TEST_F(NapiTestFixture, IsErrorNullEnv) {
  bool result = false;
  EXPECT_EQ(
      napi_invalid_arg, napi_is_error(nullptr, nullptr, &result));
}

TEST_F(NapiTestFixture, IsErrorNullValue) {
  bool result = false;
  EXPECT_EQ(
      napi_invalid_arg, napi_is_error(env_, nullptr, &result));
}
...
TEST_F(NapiTestFixture, IsErrorReturnsFalseForString) {
  ...
  napi_value val;
  ASSERT_EQ(
      napi_ok, napi_create_string_utf8(env_, "hello", 5, &val));
...
TEST_F(NapiTestFixture, IsErrorReturnsTrueForThrownError) {
  ...
  ASSERT_EQ(
      napi_ok, napi_throw_error(env_, nullptr, "thrown error"));

  napi_value exc;
  ASSERT_EQ(
      napi_ok, napi_get_and_clear_last_exception(env_, &exc));
...
TEST_F(NapiTestFixture, CreateErrorThenThrow) {
  ...
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "deferred", 8, &msg));
  napi_value code;
  ASSERT_EQ(
      napi_ok,
      napi_create_string_utf8(env_, "ERR_DEFERRED", 12, &code));

  napi_value error;
  ASSERT_EQ(
      napi_ok, napi_create_error(env_, code, msg, &error));
