...
TEST_F(NapiTestFixture, HandleScopeAPI_CloseWithoutOpen) {
  ...
  EXPECT_EQ(
      napi_handle_scope_mismatch, napi_close_handle_scope(env_, dummy));
}
...
TEST_F(NapiTestFixture, HandleScope_AddMultipleValues) {
  ...
  napi_value h3 =
      env_->addToCurrentScope(HermesValue::encodeBoolValue(true));
...
TEST_F(NapiTestFixture, HandleScope_AddNull) {
  ...
  napi_value handle =
      env_->addToCurrentScope(HermesValue::encodeNullValue());
  ASSERT_NE(nullptr, handle);
...
TEST_F(NapiTestFixture, EscapableScope_OpenNullEnv) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_open_escapable_handle_scope(nullptr, nullptr));
}
...
TEST_F(NapiTestFixture, EscapableScope_OpenNullResult) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_open_escapable_handle_scope(env_, nullptr));
  closeScope(env_, outer);
}
...
TEST_F(NapiTestFixture, EscapableScope_OpenWithoutParent) {
  ...
  EXPECT_EQ(
      ...
      napi_open_escapable_handle_scope(env_, &esc));
}
...
TEST_F(NapiTestFixture, EscapableScope_CloseNullEnv) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_close_escapable_handle_scope(nullptr, esc));
...
TEST_F(NapiTestFixture, EscapableScope_CloseNullScope) {
  EXPECT_EQ(
      napi_invalid_arg,
      napi_close_escapable_handle_scope(env_, nullptr));
}
...
TEST_F(NapiTestFixture, EscapableScope_CloseMismatch) {
  ...
  EXPECT_EQ(
      napi_handle_scope_mismatch,
      napi_close_escapable_handle_scope(env_, esc));
...
TEST_F(NapiTestFixture, EscapeHandle_CalledTwice) {
  ...
  EXPECT_EQ(
      ...
      napi_escape_handle(env_, esc, v2, &escaped2));
...
TEST_F(NapiTestFixture, EscapeHandle_NullArgs) {
  ...
  // Null env.
  EXPECT_EQ(
      napi_invalid_arg, napi_escape_handle(nullptr, esc, v, &escaped));

  // Null scope.
  EXPECT_EQ(
      napi_invalid_arg, napi_escape_handle(env_, nullptr, v, &escaped));

  // Null escapee.
  EXPECT_EQ(
      napi_invalid_arg, napi_escape_handle(env_, esc, nullptr, &escaped));
...
TEST_F(NapiTestFixture, EscapeHandle_ScopeMismatch) {
  ...
  EXPECT_EQ(
      napi_handle_scope_mismatch,
      napi_escape_handle(env_, esc, v, &escaped));
