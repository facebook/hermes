...
TEST_F(NapiTestFixture, CreateReference_NullEnv) {
  EXPECT_EQ(napi_invalid_arg, napi_create_reference(nullptr, nullptr, 0, nullptr));
}
...
TEST_F(NapiTestFixture, GetReferenceValue_NullRef) {
  napi_value result = nullptr;
  EXPECT_EQ(
      napi_invalid_arg, napi_get_reference_value(env_, nullptr, &result));
}
  ...
  napi_ref ref = nullptr;
  {
    ...
    ASSERT_EQ(
        ...
        napi_create_string_utf8(
            env_, "test string", NAPI_AUTO_LENGTH, &str));
...
TEST_F(NapiTestFixture, Reference_StrongStringSurvivesGC) {
  ...
  napi_ref ref = nullptr;
  {
    ...
    ASSERT_EQ(
        napi_ok,
        napi_create_string_utf8(
            env_, "strong string", NAPI_AUTO_LENGTH, &str));
