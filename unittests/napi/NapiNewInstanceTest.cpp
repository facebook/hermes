...
TEST_F(NapiNewInstanceTest, NullConstructor) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_new_instance(env_, nullptr, 0, nullptr, &result));
...
TEST_F(NapiNewInstanceTest, NullResult) {
  ...
  EXPECT_EQ(
      ...
      napi_create_function(
          env_, "Ctor", NAPI_AUTO_LENGTH, cb, nullptr, &func));

  EXPECT_EQ(
      napi_invalid_arg,
      napi_new_instance(env_, func, 0, nullptr, nullptr));
...
TEST_F(NapiNewInstanceTest, NonCallable) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_new_instance(env_, num, 0, nullptr, &result));
...
TEST_F(NapiNewInstanceTest, NonCallableObject) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_new_instance(env_, obj, 0, nullptr, &result));
...
TEST_F(NapiNewInstanceTest, ConstructNoArgs) {
  ...
  napi_callback cb = [](napi_env env,
    ...
    napi_get_cb_info(
        env, info, nullptr, nullptr, &thisArg, nullptr);
  ...
  EXPECT_EQ(
      napi_ok,
      napi_create_function(
          env_, "Foo", NAPI_AUTO_LENGTH, cb, nullptr, &ctor));

  napi_value instance = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_new_instance(env_, ctor, 0, nullptr, &instance));
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(env_, instance, "x", &xVal));
  double x = 0;
...
TEST_F(NapiNewInstanceTest, ConstructWithArgs) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_new_instance(env_, ctor, 2, args, &instance));
  ASSERT_NE(nullptr, instance);

  napi_value sumVal = nullptr;
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(env_, instance, "sum", &sumVal));
  double sum = 0;
...
TEST_F(NapiNewInstanceTest, NewTargetIsSet) {
  ...
  EXPECT_EQ(
      ...
      napi_get_named_property(
          env_, instance, "hasNew", &hasNewVal));
  bool hasNew = false;
  EXPECT_EQ(
      napi_ok, napi_get_value_bool(env_, hasNewVal, &hasNew));
  EXPECT_TRUE(hasNew);
...
TEST_F(NapiNewInstanceTest, ReturningObjectOverridesThis) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(
          env_, instance, "custom", &customVal));
  double custom = 0;
  EXPECT_EQ(
      napi_ok, napi_get_value_double(env_, customVal, &custom));
...
TEST_F(NapiNewInstanceTest, ReturningNonObjectUsesThis) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(
          env_, instance, "fromThis", &fromThisVal));
  double fromThis = 0;
  EXPECT_EQ(
      napi_ok,
      napi_get_value_double(env_, fromThisVal, &fromThis));
...
TEST_F(NapiNewInstanceTest, PrototypeChainIsCorrect) {
  ...
  EXPECT_EQ(
      ...
      napi_set_named_property(
          env_, proto, "inherited", protoVal));

  // Set the prototype on the constructor.
  EXPECT_EQ(
      napi_ok,
      napi_set_named_property(env_, ctor, "prototype", proto));
  ...
  EXPECT_EQ(
      ...
      napi_create_string_utf8(
          env_, "own", NAPI_AUTO_LENGTH, &ownKey));
  EXPECT_EQ(
      napi_ok,
      napi_has_own_property(env_, instance, ownKey, &hasOwn));
  EXPECT_TRUE(hasOwn);
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(
          env_, instance, "inherited", &inheritedVal));
  double inherited = 0;
  EXPECT_EQ(
      napi_ok,
      napi_get_value_double(env_, inheritedVal, &inherited));
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_prototype(env_, instance, &instanceProto));
  ...
  EXPECT_EQ(
      ...
      napi_get_value_double(
          env_, protoInherited, &protoInheritedVal));
...
TEST_F(NapiNewInstanceTest, PendingExceptionBlocksConstruct) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_create_function(
          env_, nullptr, 0, cb, nullptr, &ctor));
...
TEST_F(NapiNewInstanceTest, ConstructorThrowsSetsException) {
  ...
  EXPECT_EQ(
      napi_ok, napi_is_exception_pending(env_, &isPending));
...
TEST_F(NapiNewInstanceTest, ArgcNonZeroArgvNull) {
  ...
  EXPECT_EQ(
      napi_invalid_arg,
      napi_new_instance(env_, ctor, 1, nullptr, &result));
...
TEST_F(NapiNewInstanceTest, ConstructReturningUndefined) {
  ...
  EXPECT_EQ(
      napi_ok,
      napi_get_named_property(
          env_, instance, "marker", &markerVal));
  double marker = 0;
  EXPECT_EQ(
      napi_ok, napi_get_value_double(env_, markerVal, &marker));
