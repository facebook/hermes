...
TEST_F(JSLibTest, ObjectToStringTest) {
  ...
  EXPECT_STRINGPRIM(
      "[object Object]", toString(runtime, obj)->getHermesValue());
}
...
TEST_F(JSLibTest, ObjectGetPrototypeOfTest) {
  ...
  EXPECT_CALLRESULT_DOUBLE(
      10, JSObject::getNamed(obj2Proto, runtime, *prop1ID));
}
...
TEST_F(JSLibTest, ObjectDefinePropertyTest) {
...

  {
    ...
    auto accessor = JSObject::getNamedSlotValue(objectCons.get(), desc);
    ASSERT_TRUE(accessor.isPointer());

    auto accessorPtr = dyn_vmcast_or_null<PropertyAccessor>(accessor);
    ASSERT_NE(accessorPtr, nullptr);
    EXPECT_EQ(
        accessorPtr->getter, vmcast<Callable>(toStringFn.getHermesValue()));
    EXPECT_EQ(
        accessorPtr->setter, vmcast<Callable>(toStringFn.getHermesValue()));
...
TEST_F(JSLibTest, ObjectDefinePropertiesTest) {
  ...
  // Verify the first property.
  {
    ...
    EXPECT_TRUE(desc.flags.configurable);
    EXPECT_FALSE(desc.flags.writable);
    EXPECT_EQ(JSObject::getNamedSlotValue(obj.get(), desc).getDouble(), 123);
  }
  ...
  // Verify the second property.
  {
    ...
    EXPECT_TRUE(JSObject::getNamedSlotValue(obj.get(), desc).isNull());
  }
}
...
TEST_F(JSLibTest, MockedEnvironmentTest) {
...

  {
    ...
    auto hv = JSDate::getPrimitiveValue(valAsObj);
