...
/// Testing that the ContiguousProvider frees and recycles freed space
/// correctly. This relies on the implementation details that we always allocate
/// at the lowest-address free space and correctly free the space when the
/// storage is deleted.
TEST(StorageProviderTest, ContiguousProviderFreeTest) {
  auto provider =
      GetStorageProvider(StorageProviderType::ContiguousVAProvider, SIZE * 10);
  ...
  // This can't be allocated at the address of s1 since it won't fit.
  size_t sz3 = SIZE * 4;
  result = provider->newStorage(sz3);
  ASSERT_TRUE(result);
  auto *s3 = *result;

  // This should reuse the previously freed space.
  result = provider->newStorage(sz1);
  ASSERT_TRUE(result);
  ASSERT_EQ(s1, *result);

  // No more space.
  ASSERT_FALSE(provider->newStorage(sz3));
