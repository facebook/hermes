...
TEST(BigIntTest, getNumericValueDigitsTest) {
  // Decimal strings.
  successfulParseTest("0", "0", Radix(10));
  successfulParseTest("1", "1", Radix(10));
  successfulParseTest("12", "12", Radix(10));

  // Binary strings.
  successfulParseTest("0b0", "0", Radix(2));
  successfulParseTest("0b1", "1", Radix(2));

  // octal strings.
  successfulParseTest("0o0", "0", Radix(8));
  successfulParseTest("0o1", "1", Radix(8));

  // hex strings.
  successfulParseTest("0x0", "0", Radix(16));
  successfulParseTest("0x1", "1", Radix(16));

  // Failure cases
  parsingFailureTest(""); // empty string
  parsingFailureTest("1a"); // invalid digit

  parsingFailureTest("0b"); // missing digits and suffix
  parsingFailureTest("0b12"); // invalid digit

  parsingFailureTest("0o"); // missing digits and suffix
  parsingFailureTest("0o119"); // invalid digit

  parsingFailureTest("0x"); // missing digits and suffix
  parsingFailureTest("0x11G"); // invalid digit

  // Separators
  parsingFailureTest("0x01_2345_6789_ABCD_EFab_cdef");
  parsingFailureTest("0X01_2345_6789_ABCD_EFab_cdef");
  parsingFailureTest("0o01_234_567");
  parsingFailureTest("0O01_234_567");
  parsingFailureTest("1_2");
  parsingFailureTest("1_2_3_4_5_6_7_8_9_0_1");
  parsingFailureTest("1234_5678");
  parsingFailureTest("1234_567_9");
  parsingFailureTest("0b1010_1111");
  parsingFailureTest("0B1010_1111");
}
...
// This class is used to format the output of the CHECK_* macros below so the
// 64-bit payloads are printed as hex values.
class Double {
  uint64_t bits_;

 public:
  // Constructs a Double from a bit pattern.
  template <typename T, std::enable_if_t<std::is_unsigned_v<T>> * = nullptr>
  constexpr /*implicit*/ Double(T value) : bits_(value) {}

  // Constructs a Double from a floating point value.
  template <
      typename T,
      std::enable_if_t<std::is_floating_point_v<T>> * = nullptr>
  /*implicit*/ Double(T value)
      : Double(llvh::bit_cast<uint64_t>(static_cast<double>(value))) {}

  // Used by the EXPECT_EQ macros to compare the operands for equality.
  bool operator==(Double rhs) const {
    return bits_ == rhs.bits_;
  }

  // Global function used to output bits_ as a hex number.
  friend std::ostream &operator<<(std::ostream &out, const Double &d) {
    // Preserves out's formatting flags.
    class OutFlagsPreserver {
     public:
      OutFlagsPreserver(std::ostream &o) : out_(o), flags_(o.flags()) {}
      ~OutFlagsPreserver() {
        out_.flags(flags_);
      }
    ...
    } outFlagsPreserver(out);

    out << "0x" << std::hex << d.bits_;
    return out;
  }
};
...
TEST(BigIntTest, toDouble) {
  ...
  // Bit patterns representing the signs for positive and negative IEEE 754
  // 64-bit floating point values.
  constexpr uint64_t positive = 0ull;
  constexpr uint64_t negative = 0x8000000000000000;

  // Returns the biased exponent that represents value already shifted to the
  // correct bit. EXPECTs that value is within the proper range for 64-bit IEEE
  // 754 floating points.
  auto exp = [](int32_t value) {
    EXPECT_LE(0, value + 1023);
    EXPECT_LE(value + 1023, 2047);
    return static_cast<uint64_t>(value + 1023) << 52ull;
  };

  // Returns value. EXPECTs that value fits in 52 bits (the size of the mantissa
  // in an IEEE 754 64-bit floating point).
  auto mantissa = [](uint64_t value) {
    EXPECT_EQ(value & ((1ull << 52ull) - 1ull), value);
    return value;
  };

  // Returns a Double with the given sign, exponent, and mantissa.
  auto makeDouble = [](uint64_t s, uint64_t e, uint64_t m) {
    return Double{s | e | m};
  };

  // Converts the given BigInt to double. EXPECTs that bigint::toDouble returns
  // successfully.
  auto toDouble = [](ImmutableBigIntRef bigint) {
    double ret;
    bigint::OperationStatus s = bigint::toDouble(ret, bigint);
    EXPECT_EQ(bigint::OperationStatus::RETURNED, s);
    return Double{llvh::bit_cast<uint64_t>(ret)};
  };

  // Converts a LeftToRightVector to an Immutable BigInt ref.
  std::vector<BigIntDigitType> buffer;
  auto toImmutableRef = [&buffer](LeftToRightVector &&v) {
    buffer.resize(
        std::max<uint32_t>(1u, numDigitsForSizeInBytes(v.data.size())));
...
// Include the tests as generated by gen-bigint-todouble-tests.py
#include "BigIntSupportTest_ToDouble.inc"
}
